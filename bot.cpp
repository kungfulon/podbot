...
void EstimateNextFrameDuration (void)
{
   ...
   else if (msecval > 100.0f)
      msecval = 100.0f; // don't allow it to last longer than 100 milliseconds either
...
void BotSpawnInit (bot_t *pBot)
{
   ...
   pBot->f_wpt_timeset = gpGlobals->time + 5.0f;
   pBot->f_wpt_tim_str_chg = -1.0f; // KWo - 02.11.2009
   ...
   pBot->fTimeTeamOrder = 0.0f;

   char szBotModelName[64];
   strncpy_s (szBotModelName, sizeof(szBotModelName), (INFOKEY_VALUE (GET_INFOKEYBUFFER (pBot->pEdict), "model")), sizeof (szBotModelName) - 1);
   strncpy_s (pBot->sz_BotModelName, sizeof(pBot->sz_BotModelName), szBotModelName, sizeof (szBotModelName));

   pBot->fMinSpeed = 260.0f;
...
//   pBot->v_prev_origin = Vector (9999.0f, 9999.0f, 9999.0f);
   ...
   pBot->prev_time = gpGlobals->time + g_f_cv_FreezeTime + 1.0f;  // KWo - 02.01.2010
   pBot->f_moved_distance = 15.0f;             // KWo - 24.05.2010

   pBot->bCanDuck = FALSE;
   pBot->f_view_distance = 9999.0f;     // KWo - 30.05.2010
   pBot->f_maxview_distance = 9999.0f;  // KWo - 30.05.2010

   pBot->fButtonPushTime = 0.0f;
   pBot->fButtonNoticedTime = 0.0f;     // KWo - 14.07.2006
   ...
   pBot->f_itemcheck_time = 0.0f;
   pBot->f_timeDoorOpen = 0.0f;
   pBot->f_timeHitDoor =  0.0f;            // KWo - 25.04.2006
   ...
   pBot->fNoCollTime = gpGlobals->time + g_f_cv_FreezeTime + 1.0f;  // KWo - 02.01.2010
   ...
   pBot->fTimeNextTracking = 0.0f;
   pBot->bEnemyReachable = FALSE;         // KWo - 10.09.2008
   pBot->fEnemyReachableTimer = 0.0f;      // KWo - 10.09.2008
   ...
   pBot->f_UsageLiftTime = 0.0f;           // KWo - 22.04.2006
   ...
   pBot->f_enemy_surprise_time = 0.0f;     // KWo - 25.08.2006
   pBot->f_bot_see_enemy_time = 0.0f;
   pBot->f_bot_see_new_enemy_time = 0.0f;  // KWo - 29.04.2008

   pBot->oldcombatdesire = 0.0f;
   pBot->fEnemyOriginUpdateTime = 0.0f;    // KWo - 26.03.2007
   pBot->fLastSeenEnOrgUpdateTime = 0.0f;  // KWo - 05.05.2007
   pBot->fLastHeardEnOrgUpdateTime = 0.0f; // KWo - 05.05.2007
   pBot->fShootThruHeardCheckTime = 0.0f;  // KWo - 05.05.2007
   pBot->fShootThruSeenCheckTime = 0.0f;   // KWo - 23.03.2008
   pBot->bShootThruSeen = FALSE;          // KWo - 05.05.2007
   pBot->bShootThruHeard = FALSE;         // KWo - 05.05.2007
   pBot->fChangeAimDirectionTime = 0.0f;   // KWo - 09.12.2007
   pBot->randomized_angles = g_vecZero;   // KWo - 12.03.2010
   pBot->randomize_angles_time = 0.0f;     // KWo - 30.12.2009

   pBot->player_target_time =  0.0f;       // KWo - 30.12.2009
   ...
   pBot->f_goal_value = 0.0f; // KWo - 28.12.2009
   ...
   pBot->fTimeHostageRescue = 0.0f; // KWo - 27.02.2007

   pBot->bIsReloading = FALSE;
   pBot->f_shoot_time = 0.0f; // KWo - 23.10.2006
   pBot->fTimeSilencerSwitch = 0.0f; // KWo - 08.01.2007
   pBot->fTimeLastFired = 0.0f;
   pBot->iBurstShotsFired = 0;
   pBot->fTimeFirePause = 0.0f;
   pBot->fTimeWeaponSwitch = 0.0f;     // KWo - 23.12.2006
   ...
   pBot->f_blind_time = 0.0f;
   pBot->f_blindmovespeed_forward = 0.0f;  // KWo - 13.08.2008
   pBot->f_blindmovespeed_side = 0.0f;     // KWo - 13.08.2008
   pBot->f_blindmove_time = 0.0f;          // KWo - 13.08.2008
   pBot->bShootLastPosition = FALSE;      // KWo - 13.08.2008
   pBot->bMadShoot = FALSE;               // KWo - 13.08.2008

   pBot->f_jumptime = 0.0f;
   pBot->f_ducktime = 0.0f;             // KWo - 20.10.2006
   pBot->f_falldowntime = 0.0f;         // KWo - 05.03.2010
   pBot->fNextCampDirectionTime = 0.0f; // KWo - 27.02.2007
   pBot->fTimeCamping = 0.0f;           // KWo - 27.02.2007
   pBot->iCampButtons = 0;             // KWo - 17.02.2008

   pBot->f_sound_update_time = gpGlobals->time;
   pBot->f_heard_sound_time = 0.0f;

   pBot->fTimePrevThink = gpGlobals->time;
   pBot->fTimePrevThink2 = gpGlobals->time;
   pBot->fTimeFrameInterval = 0.0f;

   pBot->i_msecval = 0;                // KWo - 17.03.2007
   pBot->f_msecvalrest = 0.0f;          // KWo - 17.03.2007
...
//   pBot->f_buy_time = gpGlobals->time + RANDOM_FLOAT (0.5f, (g_f_cv_FreezeTime >= 2.f)? g_f_cv_FreezeTime : 2.f);
   pBot->f_buy_time = gpGlobals->time + RANDOM_FLOAT (0.5, 1.0f);

   pBot->f_zoomchecktime = 0.0f;
   pBot->f_reloadingtime = 0.0f;
   pBot->iNumWeaponPickups = 0;

   pBot->f_StrafeSetTime = 0.0f;
   pBot->byCombatStrafeDir = 0;
   pBot->byFightStyle = 0;
   pBot->f_lastfightstylecheck = 0.0f;
   ...
   bottask_t TempTask = {NULL, NULL, TASK_NORMAL, TASKPRI_NORMAL, -1, 0.0f, TRUE};
      ...
      if (g_i_cv_detailnames)
      {
         if (g_i_cv_detailnames == 1) // use prefix and sufix
         {
            if (bot_personality == 0)
               _snprintf_s (c_name, sizeof (c_name), sizeof (c_name) - 1, "[POD]%s (%d)", (g_pszBotNames + name_index)->name, skill);
            else if (bot_personality == 1)
               _snprintf_s (c_name, sizeof (c_name), sizeof (c_name) - 1, "[P*D]%s (%d)", (g_pszBotNames + name_index)->name, skill);
            else
               _snprintf_s (c_name, sizeof (c_name), sizeof (c_name) - 1, "[P0D]%s (%d)", (g_pszBotNames + name_index)->name, skill);
         }
         if (g_i_cv_detailnames == 2) // use prefix only
         {
            if (bot_personality == 0)
               _snprintf_s (c_name, sizeof (c_name), sizeof (c_name) - 1, "[POD]%s", (g_pszBotNames + name_index)->name);
            else if (bot_personality == 1)
               _snprintf_s (c_name, sizeof (c_name), sizeof (c_name) - 1, "[P*D]%s", (g_pszBotNames + name_index)->name);
            else
               _snprintf_s (c_name, sizeof (c_name), sizeof (c_name) - 1, "[P0D]%s", (g_pszBotNames + name_index)->name);
         }
         if (g_i_cv_detailnames == 3) // use sufix only
         {
            _snprintf_s (c_name, sizeof (c_name), sizeof (c_name) - 1, "%s (%d)", (g_pszBotNames + name_index)->name, skill);
         }
      }
      else
         strncpy_s (c_name, sizeof(c_name), (g_pszBotNames + name_index)->name, sizeof (c_name) - 1);
   }
   ...
   else
   {
      ...
      if (g_i_cv_detailnames) // KWo - 22.03.2008 - added new meaning of g_i_cv_detailnames
      {
         if ((g_i_cv_detailnames == 1) || (g_i_cv_detailnames == 2))
         {
            ...
            {
               strncpy_s (c_name, sizeof(c_name), bot_name, sizeof(c_name) - 1);
            ...
            else // KWo - 07.07.2008
            {
               if ((strstr (bot_name, "(") != NULL) && (strstr (bot_name, ")") != NULL)) // the bot already had a sufix
               {
                  ...
                  {
                     if (bot_personality == 0)
                        _snprintf_s (c_name, sizeof (c_name), sizeof (c_name) - 1, "[POD]%s", bot_name);
                     else if (bot_personality == 1)
                        _snprintf_s (c_name, sizeof (c_name), sizeof (c_name) - 1, "[P*D]%s", bot_name);
                     else
                        _snprintf_s (c_name, sizeof (c_name), sizeof (c_name) - 1, "[P0D]%s", bot_name);
                  }
               }
               else
               {
                  ...
                  {
                     if (bot_personality == 0)
                        _snprintf_s (c_name, sizeof (c_name), sizeof (c_name) - 1, "[POD]%s (%d)", bot_name, skill);
                     else if (bot_personality == 1)
                        _snprintf_s (c_name, sizeof (c_name), sizeof (c_name) - 1, "[P*D]%s (%d)", bot_name, skill);
                     else
                        _snprintf_s (c_name, sizeof (c_name), sizeof (c_name) - 1, "[P0D]%s (%d)", bot_name, skill);
                  }
         ...
         else // use sufix only
         {
            if ((strstr (bot_name, "(") == NULL) || (strstr (bot_name, ")") == NULL)) // KWo - 07.07.2008 the bot didn't have yet a sufix
               _snprintf_s (c_name, sizeof (c_name), sizeof (c_name) - 1, "%s (%d)", bot_name, skill);
            else
               _snprintf_s (c_name, sizeof (c_name), sizeof (c_name) - 1, "%s", bot_name);
         }
      }
      else
         strncpy_s (c_name, sizeof(c_name), bot_name, sizeof (c_name) - 1);
...
//   SET_CLIENT_KEYVALUE (index + 1, infobuffer, "cl_lc", "1"); // KWo - 14.09.2011 - thanks to Immortal_BLG
   ...
   _snprintf_s (pBot->name, sizeof (pBot->name), sizeof (pBot->name) - 1, "%s", STRING (pBot->pEdict->v.netname));
   pBot->is_used = TRUE;
   ...
   pBot->SaytextBuffer.fChatDelay = RANDOM_FLOAT (3.0f, 10.0f);    // KWo - 10.03.2013
   ...
   case 0:
      ...
      // Psycho
   case 1:
      pBot->fBaseAgressionLevel = RANDOM_FLOAT (0.7, 1.0f);
      pBot->fBaseFearLevel = RANDOM_FLOAT (0.0f, 0.4);
      break;
      // Coward
   case 2:
      pBot->fBaseAgressionLevel = RANDOM_FLOAT (0.0f, 0.4);
      pBot->fBaseFearLevel = RANDOM_FLOAT (0.7, 1.0f);
      break;
   }
   ...
   if (skill == 100)
     pBot->fOffsetUpdateTime = 0.0f;  // enemy origin should be updated in aiming system all the time
   else
   {
      // botskill 99 -> update time = 0.05s, botskill 1 -> update time = 0.25s
      pBot->fOffsetUpdateTime = 0.25 + (skill-1) *(0.05-0.25)/(99.0f);  // KWo - 27.04.2006
   }
...
bool BotItemIsVisible (bot_t *pBot, Vector vecDest, char *pszItemName, bool bBomb)
{
   ...
   if (tr.flFraction < 1.0f) // KWo - 28.08.2008
   {
...
//      ALERT(at_logged, "[DEBUG] Bot %s needs to avoid grenade.\n", pBot->name);
   ...
   if (!FNullEnt (pSmoke)) // KWo - 13.09.2008
   {
      ...
      if (iInField < (pEdict->v.fov * 1.0f - 5.0f)) // more wide - even +/-85 degress in this case...
      {
...
int BotRateGroundWeapon (bot_t *pBot, edict_t *pent)
{
   ...
   strncpy_s (szModelName, sizeof(szModelName), STRING (pent->v.model), sizeof (szModelName) - 1);
...
bool BotFindBreakable (bot_t *pBot)
{
   ...
   for (int i = 0; i < 5; ++i)
   {
      ...
      if (FNullEnt (tr.pHit)) // KWo - 28.01.2006
         continue;
      if ((tr.flFraction != 1.0f) && ((pEdict->v.origin - tr.vecEndPos).Length() < 120.0f)
...
bool IsDeadlyDropAtPos (bot_t *pBot, Vector vecTargetPos) // KWo - 02.04.2010
{
   ...
   height = tr.flFraction * 1000.0f; // height from ground at goal position
...
*/
   ...
   last_height = tr.flFraction * 1000.0f; // height from ground at goal position
   distance = (vecBot - v_check).Length (); // distance from goal

   if ((distance <= 30.0f) && (last_height > 150.0f)) // KWo - 02.04.2010
      return (TRUE);

   while (distance > 30.0f) // KWo - 30.10.2009
   {
      // move 30 units closer to the bot...
      v_check = v_check - (v_direction * 30.0f); // KWo - 02.04.2010
...
void BotFindItem (bot_t *pBot)
{
   ...
   if (!FNullEnt (pBot->pBotPickupItem))
   {
      ...
      while (!FNullEnt (pent = FIND_ENTITY_IN_SPHERE (pent, pEdict->v.origin, 500)))
      {
         ...
         _snprintf_s (vclassname, sizeof (vclassname), sizeof (vclassname) - 1, "%s", STRING (pent->v.classname)); // KWo - 25.12.2006
   ...
   while (!FNullEnt (pent = FIND_ENTITY_IN_SPHERE (pent, pEdict->v.origin, 500)))
   {
      ...
      if ((pent->v.effects & EF_NODRAW) || (pent == pBot->pItemIgnore[0])
          ...
          || (fabs(pent->v.origin.z - pEdict->v.origin.z) > 100.0f))  // KWo - 08.04.2016
      ...
      {
         _snprintf_s (vclassname, sizeof (vclassname), sizeof (vclassname) - 1, "%s", STRING (pent->v.classname)); // KWo - 13.05.2006
         _snprintf_s (vmodelname, sizeof (vmodelname), sizeof (vmodelname) - 1, "%s", STRING (pent->v.model)); // KWo - 18.12.2006
      ...
      if (bCanPickup)
      {
         ...
         if (fDistance < fMinDistance)
         {
            ...
            {
               if (pBot->bIsChickenOrZombie || ((pBot->bIsVIP) && (!bSecondaryAmmo))
                   || (((BotRateGroundWeapon (pBot, pent) <= 0)
                  ...
                  bCanPickup = FALSE;
               else if (((pBot->bIsVIP) && (bPrimaryAmmo)) || ((bArmor) && (pEdict->v.armorvalue > 60.0f))
                  || ((bMedkit) && (pEdict->v.health > 60.0f)))
                  bCanPickup = FALSE;
            ...
            else if (pBot->bot_team == TEAM_CS_TERRORIST)
            {
               if (iPickType == PICKUP_PLANTED_C4)
               {
                  ...
                  if ((!pBot->bDefendedBomb) && (!pBot->bIsChickenOrZombie)) // KWo - 18.01.2011
                  {
                     ...
                     if (fTimeMidBlowup - fTraveltime - g_fTimeBombPlanted > 0.0f) // KWo - 16.06.2008
                     {
            ...
            else
            {
               ...
               else if (iPickType == PICKUP_PLANTED_C4)
               {
                  ...
                  for (i = 0; i < gpGlobals->maxClients; i++)
                  {
                     ...
                     if ((fDistance2 < 60) && (fabs(pPlayer->v.origin.z - vecEntityOrigin.z) < 40.0f) && (fDistance > fDistance2)) // KWo - 05.09.2008
                     {
                        bCanPickup = FALSE;
                        ...
                        if ((!pBot->bDefendedBomb) && (!pBot->bIsChickenOrZombie)) // KWo - 18.01.2011
                        {
                           ...
                           if (fTimeBlowup - g_fTimeBombPlanted - fTraveltime > 0.0f) // KWo - 16.06.2008
                           {
                              pBot->bDefendedBomb = TRUE; // KWo - 23.03.2008
                              // Push camp task on to stack
               ...
               else if (iPickType == PICKUP_DROPPED_C4)
               {
                  ...
                  if (((pBot->pEdict->v.health * RANDOM_LONG (1, 100)) > 3000.0f)
                     && (BotHasCampWeapon(pBot)) && (pBot->current_weapon.iAmmo1 > 0)
                     && (pBot->fTimeCamping + 10.0f < gpGlobals->time)
                     && (!pBot->bIsChickenOrZombie)) // KWo - 18.01.2011
                  {
                     // Push camp task on to stack
                     float fCampTime = RANDOM_FLOAT (30.0f, 60.0f); // KWo - 23.03.2008
   ...
   if (pPickupEntity != NULL)
   {
      ...
      if (vecPickupOrigin.z > GetGunPosition (pEdict).z + ((pBot->iPickupType == PICKUP_HOSTAGE)? 50.0f : 20.0f)) // KWo - 08.03.2010
      {
         pBot->pBotPickupItem = NULL;
         pBot->iPickupType = PICKUP_NONE;
...
void BotCheckMessageQueue (bot_t *pBot)
{
   ...
   switch (iCurrentMSG)
   {
      ...
      case MSG_CS_RADIO:
      {
         ...
         if (g_rgfLastRadioTime[pBot->bot_team - 1] + 3.0f < gpGlobals->time)
         {
            char szReport[80];
...
/*
                ...
                && (pBot->iRadioSelect != RADIO_FOLLOWME))  // KWo - 27.04.2006
            {
               if ((pBot->iRadioSelect == g_rgfLastRadio[pBot->bot_team - 1])
                   && (g_rgfLastRadioTime[pBot->bot_team - 1] + 1.5f > gpGlobals->time))
...
*/
            if (pBot->iRadioSelect == RADIO_REPORTINGIN)
            {
               ...
               switch (iTask)
               {
                  case TASK_NORMAL:
                  {
                     if (iWPT != -1)
                     {
                        if (paths[iWPT]->flags & W_FL_GOAL)
                           _snprintf_s (szReport, iSize, iSize - 1, "Heading for a Map Goal!");
                        else if (paths[iWPT]->flags & W_FL_RESCUE)
                           _snprintf_s (szReport, iSize, iSize - 1, "Heading to Rescue Point");
                        else if (paths[iWPT]->flags & W_FL_CAMP)
                           _snprintf_s (szReport, iSize, iSize - 1, "Moving to Camp Spot");
                        else
                           _snprintf_s (szReport, iSize, iSize - 1, "Roaming around");
                     }
                     else
                        _snprintf_s (szReport, iSize, iSize - 1, "Roaming around");
                     break;
                  ...
                  case TASK_MOVETOPOSITION:
                     _snprintf_s (szReport, iSize, iSize - 1, "Moving to position");
                     break;
                  case TASK_FOLLOWUSER:
                     if (!FNullEnt (pBot->pBotUser))
                        _snprintf_s (szReport, iSize, iSize - 1, "Following %s", STRING (pBot->pBotUser->v.netname));
                     break;
                  case TASK_WAITFORGO:
                     _snprintf_s (szReport, iSize, iSize - 1, "Waiting for GO!");
                     break;
                  case TASK_CAMP:
                     _snprintf_s (szReport, iSize, iSize - 1, "Camping...");
                     break;
                  case TASK_PLANTBOMB:
                     _snprintf_s (szReport, iSize, iSize - 1, "Planting the Bomb!");
                     break;
                  case TASK_DEFUSEBOMB:
                     _snprintf_s (szReport, iSize, iSize - 1, "Defusing the Bomb!");
                     break;
                  case TASK_ATTACK:
                     if (!FNullEnt (pBot->pBotEnemy))
                        _snprintf_s (szReport, iSize, iSize - 1, "Attacking %s", STRING (pBot->pBotEnemy->v.netname));
                     break;
                  case TASK_ENEMYHUNT:
                     if (!FNullEnt (pBot->pLastEnemy))
                        _snprintf_s (szReport, iSize, iSize - 1, "Hunting %s", STRING (pBot->pLastEnemy->v.netname));
                     break;
                  case TASK_SEEKCOVER:
                     _snprintf_s (szReport, iSize, iSize - 1, "Fleeing from Battle");
                     break;
                  case TASK_HIDE:
                     _snprintf_s (szReport, iSize, iSize - 1, "Hiding from Enemy");
                     break;
                  default:
                     _snprintf_s (szReport, iSize, iSize - 1, "Nothing special here...");
                     break;
               }
            }
      ...
      case MSG_CS_SAY:
      {
         ...
         _snprintf_s (szMessage, sizeof (szMessage), sizeof (szMessage) - 1,"%s:%s", STRING (pBot->pEdict->v.netname), pBot->szMiscStrings);
         ...
         for (bot_index = 0; bot_index < gpGlobals->maxClients; bot_index++)
         {
            ...
            if (pOtherBot->is_used && !FNullEnt (pOtherBot->pEdict) && (pOtherBot != pBot))
            {
               if (!pOtherBot->bDead)
               {
                  ...
                  strncpy_s (pOtherBot->SaytextBuffer.szSayText, sizeof(pOtherBot->SaytextBuffer.szSayText), szMessage, sizeof (pOtherBot->SaytextBuffer.szSayText) - 1);
...
void BotCheckReload (bot_t *pBot)  // bluebyte
{
   ...
   if (((!pBot->bIsReloading && FNullEnt(pBot->pBotEnemy) && (weapon_defs[iId].iAmmo1 != -1)
      ...
      && (!(pBot->iStates & STATE_HEARINGENEMY)) && (pBot->f_bot_see_enemy_time + 4.0f < gpGlobals->time)
      && (pBot->f_heard_sound_time + 4.0f < gpGlobals->time)))
      || (!(pBot->iStates & STATE_SEEINGENEMY) && (pBot->f_bot_see_enemy_time + 1.0f < gpGlobals->time) && (g_bRoundEnded))
      ...
      || ((g_iAliveTs == 0) && (pBot->bot_team == TEAM_CS_COUNTER)))) // 30.09.2010
   {
      ...
      if (((pBot->current_weapon.iClip < (0.8 * iMaxClip)) || (pBot->f_spawn_time + 5.0f > gpGlobals-> time))
         && (pBot->f_reloadingtime == 0.0f) && (pBot->f_spawn_time + 2.0f < gpGlobals-> time))  // KWo - 10.07.2008
      {
         ...
         if (g_b_DebugCombat)
         {
            ...
            && (pBot->f_reloadingtime < gpGlobals->time) && (pBot->fTimeWeaponSwitch + 0.5f < gpGlobals->time)) // KWo - 10.02.2008
   {
      pBot->bIsReloading = FALSE;
      pBot->f_reloadingtime = 0.0f;
      if ((WeaponIsPrimaryGun(iId)) && (pBot->current_weapon.iAmmo1 > 0))
...
void BotBuyStuff (bot_t *pBot)
{
   ...
   if (pBot->iBuyCount == 1)
   {
      ...
      pBot->iBuyCount++;
      pBot->f_buy_time = gpGlobals->time + RANDOM_FLOAT (0.3, 0.5f);
      return;
   }
   ...
   else if (pBot->iBuyCount < 5) // KWo - 18.01.2011
   {
      ...
      pBot->iBuyCount++;
      pBot->f_buy_time = gpGlobals->time + RANDOM_FLOAT (0.2, 0.5f);
      return;
   }
   ...
   else if (pBot->iBuyCount < 8)
   {
      ...
      if (pBot->iBuyCount == 7)  // KWo - 27.05.2008
      {
         ...
         if (((((g_f_cv_skycolor > 50.0f) && (fLightLevel < 15.0f)) || ((g_f_cv_skycolor <= 50.0f) && (fLightLevel < 40.0f)))
...
//   static unsigned char *pas;
   ...
   pEdict = pBot->pEdict;
   vLastEnLookDir.x = 90.0f;      // KWo - 01.10.2010
   ...
   if ((pBot->iAimFlags & AIM_ENTITY) && (BotGetSafeTask(pBot)->iTask != TASK_PICKUPITEM))  // KWo - 27.10.2006
      ...
      && ((pBot->fLastHeardEnOrgUpdateTime >= gpGlobals->time)
          ...
          || (pBot->f_sound_update_time >= gpGlobals->time)
          || (pBot->f_bot_see_enemy_time + 2.0f < gpGlobals->time))) // KWo - 29.05.2010
   ...
   {
      if (pBot->f_bot_see_enemy_time + 1.0f > gpGlobals->time)        // KWo - 10.04.2010
      {
         pBot->fAgressionLevel += 0.05;
         if (pBot->fAgressionLevel > 1.0f)
            pBot->fAgressionLevel = 1.0f;
      }
      else if (pBot->f_bot_see_enemy_time + 5.0f < gpGlobals->time)   // KWo - 10.04.2010
      {
         ...
         if (pBot->fAgressionLevel < 0.0f)
            pBot->fAgressionLevel = 0.0f;
         if (pBot->fFearLevel < 0.0f)
            pBot->fFearLevel = 0.0f;
      }
      pBot->fNextEmotionUpdate = gpGlobals->time + 0.5f;
   }
   ...
   // We're blind and can't see anything !!
      if (pBot->f_blind_time > gpGlobals->time) // moved from BotFindEnemy function - KWo - 17.06.2018
      {
   ...
   if (pBot->f_bot_see_enemy_time + 2.0f < gpGlobals->time)        // KWo - 10.07.2008
   {
      pBot->iStates &= ~STATE_SEEINGENEMY;
      pBot->pBotEnemy = NULL;
      pBot->bShootThruSeen = FALSE;                               // KWo - 10.07.2008
   }
   else if (pBot->f_bot_see_enemy_time + 1.0f < gpGlobals->time)        // KWo - 11.01.2012
   {
...
//        && (pBot->f_bot_see_new_enemy_time + 0.5f < gpGlobals->time) // ??
        && (pBot->f_blind_time < gpGlobals->time)) // KWo - 11.07.2008
   {
      ...
      fBotOwnSoundLast = 0.0f;       // KWo - 12.10.2006
      ...
      {
         if (clients[g_i_botthink_index].fMaxTimeSoundLasting <= 0.0f)
            clients[g_i_botthink_index].fMaxTimeSoundLasting = 0.5f;
//         fBotOwnSoundLast = (clients[g_i_botthink_index].fHearingDistance * (0.2)) * (clients[g_i_botthink_index].fTimeSoundLasting - gpGlobals->time)/clients[g_i_botthink_index].fMaxTimeSoundLasting;
         if (clients[g_i_botthink_index].fTimeSoundLasting + 1.8 < gpGlobals->time)
            fBotOwnSoundLast = (0.2) * (clients[g_i_botthink_index].fHearingDistance / 1024.0f); // KWo - 12.09.2008
      }
...
*/
      pPlayer = NULL;
      fMinDistance = 9999;
      fMaxVolume = 0.0f; // KWo - 01.08.2006
      fVolume = 0.0f;
      ...
      if (!FNullEnt(pEdict->v.dmg_inflictor)) // KWo - 10.04.2010
      {
         ...
         {
            if ((clients[ind].iFlags & CLIENT_ALIVE) && (pEdict->v.dmgtime + 1.5f > gpGlobals->time)) // KWo - 19.01.2011
            {
               if ((FNullEnt(pBot->pBotEnemy))
                  || ((pBot->f_bot_see_enemy_time + 1.0f < gpGlobals->time) && (pBot->pLastEnemy != pPlayer)))
               {
                  iHearEnemyIndex = ind;
                  pPlayer = pEdict->v.dmg_inflictor;
                  fMinDistance = 500.0f;
                  fVolume = fBotOwnSoundLast * 10.0f + 500.0f;
...
/*
                  ...
                  pBot->vecLastEnemyOrigin.x += RANDOM_FLOAT(-200.0f,200.0f);
                  pBot->vecLastEnemyOrigin.y += RANDOM_FLOAT(-200.0f,200.0f);
...
*/
            ...
            else
            {
               if ((pBot->pBotEnemy == pEdict->v.dmg_inflictor)
                  ...
                  || (pBot->f_bot_see_enemy_time + 7.0f < gpGlobals->time)))    // 15.10.2011
               {
      ...
      for (ind = 0; ind < gpGlobals->maxClients; ind++)
      {
         ...
         if (clients[ind].fMaxTimeSoundLasting <= 0.0f) // KWo - 01.08.2006
            clients[ind].fMaxTimeSoundLasting = 1.0f;

         if (clients[ind].fTimeSoundLasting < gpGlobals->time) // KWo - 12.09.2008
            continue;
/*

         if (clients[ind].fTimeSoundLasting - gpGlobals->time > 0.5f)
         {
            if (fDistance < 1.0f)
               fDistance = 1.0f;
            fVolume = fHearingDistance * (1.0f - fDistance / fHearingDistance);
         }
         else if (fDistance <= 0.5f * fHearingDistance) // KWo - 12.10.2006
         {
            if (fDistance < 1.0f)
               fDistance = 1.0f;

            fVolume = fHearingDistance * (1.0f - fDistance / fHearingDistance) * (clients[ind].fTimeSoundLasting - gpGlobals->time)/clients[ind].fMaxTimeSoundLasting;
         }

         else
            fVolume = 2.0f * fHearingDistance * (1.0f - fDistance / fHearingDistance) * (clients[ind].fTimeSoundLasting - gpGlobals->time)/clients[ind].fMaxTimeSoundLasting; // KWo - 01.08.2006
...
//         if (ENGINE_CHECK_VISIBILITY (clients[iHearEnemyIndex].pEdict, pas))
      ...
      if (!FNullEnt (pPlayer))
      {
         ...
         // Didn't Bot already have an enemy ? Take this one...
            ...
            {
               if (pBot->f_bot_see_new_enemy_time + 1.0f < gpGlobals->time)             // KWo - 11.07.2008
               {
                  ...
                  pBot->pLastEnemy = pPlayer;
                  pBot->vecLastEnemyOrigin = pPlayer->v.origin;
                  pBot->vecLastEnemyOrigin.x += RANDOM_FLOAT(-200.0f,200.0f);   // KWo - 05.05.2007
                  pBot->vecLastEnemyOrigin.y += RANDOM_FLOAT(-200.0f,200.0f);   // KWo - 05.05.2007
                  pBot->fLastHeardEnOrgUpdateTime = gpGlobals->time + 1.0f;    // KWo - 05.05.2007
                  pBot->fShootThruHeardCheckTime = 0.0f;                       // KWo - 05.05.2007
//                  pBot->fChangeAimDirectionTime = gpGlobals->time + 1.0f;      // KWo - 24.02.2008
                  pBot->iAimFlags |= AIM_LASTENEMY;
                  bNewEnemy = TRUE;
               }
               else
            ...
            else
            {
               if (pPlayer == pBot->pLastEnemy)
               {
                  ...
                  {
                     pBot->vecLastEnemyOrigin = pPlayer->v.origin;
                     pBot->vecLastEnemyOrigin.x += RANDOM_FLOAT(-300.0f,300.0f);   // KWo - 05.05.2007
                     pBot->vecLastEnemyOrigin.y += RANDOM_FLOAT(-300.0f,300.0f);   // KWo - 05.05.2007
                     pBot->fLastHeardEnOrgUpdateTime = gpGlobals->time + 1.0f;    // KWo - 05.05.2007
                     pBot->iAimFlags |= AIM_LASTENEMY;
               ...
               else
               {
                  ...
                  if ((0.8 * fDistance > (pPlayer->v.origin - pEdict->v.origin).Length ())  // KWo - 16.12.2007
                   && (pBot->f_bot_see_new_enemy_time + 1.0f < gpGlobals->time))
                  {
                     if ((pBot->iStates & STATE_SEEINGENEMY) && (pBot->f_bot_see_enemy_time + 1.0f > gpGlobals->time)) // KWo - 09.04.2010
                     {
                        pBot->f_heard_sound_time = gpGlobals->time;
                        pBot->iStates |= STATE_HEARINGENEMY;
                        ...
                        && (pBot->fLastSeenEnOrgUpdateTime < gpGlobals->time))
                     {
                        ...
                        pBot->vecLastEnemyOrigin.y += RANDOM_FLOAT(-300.0f,300.0f);               // KWo - 05.05.2007
                        pBot->fLastHeardEnOrgUpdateTime = gpGlobals->time + 1.0f;                // KWo - 05.05.2007
                        pBot->fShootThruHeardCheckTime = 0.0f;
//                        pBot->f_bot_see_new_enemy_time = gpGlobals->time;                       // KWo - 15.07.2008
//                        pBot->fChangeAimDirectionTime = gpGlobals->time + 1.0f;                  // KWo - 15.07.2008
...
*/
                  ...
                  else if (pBot->f_bot_see_new_enemy_time + 1.0f >= gpGlobals->time)
                  {
                     pPlayer = NULL;
                     goto endhearing;
                  }
         ...
         if (FBoxVisible (pBot, pPlayer, &vVisPos, &cHit))
         {
            ...
            {
               pBot->f_bot_see_new_enemy_time = gpGlobals->time;
               pBot->fChangeAimDirectionTime = gpGlobals->time + 1.0f;   // KWo - 17.02.2008
            }
            pBot->iAimFlags |= AIM_LASTENEMY;
...
shootthruwcheck:
      ...
      if (!FNullEnt (pBot->pLastEnemy) && (pBot->f_heard_sound_time + 1.5f >= gpGlobals->time)
            ...
            && !(pBot->iStates & STATE_SEEINGENEMY) && (pBot->vecLastEnemyOrigin != g_vecZero)) // KWo - 13.08.2008
      {
         ...
         if (fabs(vLastEnLookDir.x) < 45.0f)     // KWo - 01.10.2010
         {
            iShootThruFreq = BotAimTab[pBot->bot_skill / 20].iHeardShootThruProb;
			   pBot->fShootThruHeardCheckTime = gpGlobals->time + 1.0f;
               ...
               && (RANDOM_LONG (1, 100) <= iShootThruFreq))
            {
               ...
               {
                  pBot->fChangeAimDirectionTime = gpGlobals->time + 1.2f;
			         pBot->bShootThruHeard = TRUE;
               }
               else
               {
                  if (pBot->bShootThruHeard) // KWo - 13.08.2008
                     pBot->fShootThruHeardCheckTime = gpGlobals->time + 15.0f;
                  pBot->bShootThruHeard = FALSE;
               }
            }
            else
      ...
      if (!FNullEnt (pBot->pLastEnemy) && (pBot->bShootThruHeard)) // KWo - 13.07.2008
      {
         pBot->iAimFlags |= AIM_LASTENEMY;
         pBot->iStates |= STATE_SUSPECTENEMY;
         if ((pBot->f_bot_see_enemy_time + 1.0f < gpGlobals->time)
            && (pBot->fLastHeardEnOrgUpdateTime < gpGlobals->time)
            && (pBot->fLastSeenEnOrgUpdateTime < gpGlobals->time))
         {
            pBot->vecLastEnemyOrigin.x += RANDOM_FLOAT (-300.0f, 300.0f);
            pBot->vecLastEnemyOrigin.y += RANDOM_FLOAT (-300.0f, 300.0f);
            pBot->fLastHeardEnOrgUpdateTime = gpGlobals->time + 1.0f;
         }
   ...
   {
      if (pBot->f_heard_sound_time + 10.0f < gpGlobals->time)
      {
         pBot->iStates &= ~STATE_HEARINGENEMY;
         pBot->bShootThruHeard = false;
         if ((FNullEnt(pBot->pBotEnemy)) && (pBot->f_bot_see_enemy_time + 10.0f < gpGlobals->time)) // KWo - 29.05.2010
         {
            pBot->vecLastEnemyOrigin = g_vecZero;
            pBot->pLastEnemy = NULL;
            pBot->iAimFlags &= ~AIM_LASTENEMY;
...
endhearing:
       ...
       && !(pBot->iAimFlags & AIM_ENEMY)) // KWo - 11.01.2012
   {
      ...
      if (((fDistance >= 300.0f) && FNullEnt(pBot->pBotEnemy)
          && (pBot->f_bot_see_enemy_time + 2.0f < gpGlobals->time) && (tr.flFraction <= 0.2f)
          && (FStrEq(STRING (tr.pHit->v.classname), "worldspawn"))
          && !(pBot->bShootThruSeen) && !(pBot->bShootThruHeard))
          || (!(pBot->bShootThruSeen) && (fabs(vLastEnLookDir.x) >= 45.0f)))     // KWo - 01.10.2010
      {
         pBot->iAimFlags &= ~AIM_LASTENEMY;
      }
   ...
   if ((pBot->f_bot_see_enemy_time + 8.0f < gpGlobals->time) && (pBot->f_heard_sound_time + 10.0f < gpGlobals->time)) // KWo - 10.07.2008
      pBot->iStates &= ~STATE_SUSPECTENEMY;
}
      ...
      /* && (strncmp ("zomb", pBot->sz_BotModelName, 4) != 0) */) // KWo - 14.06.2008
   {
      ...
      if (bWantThrowFlashbang) // KWo - 21.03.2006
      {
         ...
         if (bThrowGrenade)
         {
            ...
            TempTask.fTime = gpGlobals->time + 3.0f;

            BotPushTask(pBot, &TempTask);
            pBot->f_grenade_check_time = gpGlobals->time + 3.0f;
         }
      ...
      else if ((!FNullEnt (pBot->pLastEnemy)) && (pBot->vecLastEnemyOrigin != g_vecZero)) // KWo - 23.07.2007
      {
         if (IsAlive (pBot->pLastEnemy)) // KWo - 06.08.2006
         {
            ...
            if ((iGrenadeType == CS_WEAPON_HEGRENADE) || (iGrenadeType == CS_WEAPON_SMOKEGRENADE)) // KWo - 11.04.2010
            {
               ...
               // Too high to throw ?
               if (pBot->vecLastEnemyOrigin.z > pEdict->v.origin.z + 500.0f)
                  fDistance = 9999;
               ...
               if (pBot->iStates & STATE_SEEINGENEMY)
               {
                  ...
                  if ((iNumTeamnates == 0) && (fDistance > 500.0f) && (fDistance < 1600.0f)
                     && ((!WeaponIsPrimaryGun(iEnWepID))
                         || ((fEnemyDot < 0.7f) && (pBot->pLastEnemy->v.oldbuttons & IN_ATTACK))
                            /* || (fEnemyDot < 0.4f) */ ))
                  {
                     vPredict = pBot->pLastEnemy->v.velocity * fDistance / 400.0f;  // KWo - 18.06.2006
                     vPredict.z = 0.0f;
                     vPredict = vPredict + pBot->pLastEnemy->v.origin;

                     pBot->vecThrow = vPredict;
                     vSourceThrow = VecCheckThrow (pBot, GetGunPosition (pEdict), pBot->vecThrow, 1.0f);
                     ...
                     if (vSourceThrow == g_vecZero)
                     {
                        ...
                        if (vSourceThrow != g_vecZero) // KWo - 24.02.2008
                           pBot->vecThrow = pBot->vecThrow + Vector(0.0f, 0.0f, 110.0f);
                     }
                  ...
                  && ((pBot->f_bot_see_enemy_time + 4.0f > gpGlobals->time)
                     || (pBot->f_heard_sound_time + 4.0f > gpGlobals->time)))
               {
                  if ((iNumTeamnates == 0) && (fDistance > 500.0f) && (fDistance < 1600.0f))
                  {
                     ...
                     if (fSearchRadius < 128.0f)
                        fSearchRadius = 128.0f;
               ...
               if (bThrowGrenade)
               {
                  fRandom = RANDOM_FLOAT(0.0f,100.0f);
                  ...
                  TempTask.fDesire = TASKPRI_THROWGRENADE;
                  TempTask.fTime = gpGlobals->time + 4.0f;

                  if (bThrowGrenade) // KWo - 24.02.2006
                  {
                     pBot->f_grenade_check_time = gpGlobals->time + 6.0f;
                  }
...
// part dedicated to selecting the particular task to do...
   ...
   if (iFriendlyNum > 0)
      fTempFear = fTempFear * 0.5f;

   // Increase/Decrease Fear/Agression if Bot uses a sniping weapon
   // to be more careful
   if (BotUsesSniper (pBot))
   {
      fTempFear = fTempFear * 1.5f;
      fTempAgression = fTempAgression * 0.5f; // KWo - 26.08.2006
   }
      ...
      && (pBot->chosengoal_index >= 0) && (pBot->chosengoal_index < g_iNumWaypoints)) // KWo - 26.04.2008
   {
      if ((BotGetSafeTask(pBot)->iTask != TASK_MOVETOPOSITION) && (BotGetSafeTask(pBot)->fDesire < TASKPRI_HIDE)
         ...
         && ((paths[pBot->chosengoal_index]->origin - pEdict->v.origin).Length() < 800.0f))
      {
         if (IsGroupOfEnemies(pBot, paths[pBot->chosengoal_index]->origin))
         {
            ...
            if ((float)iEnemyNum * pBot->fFearLevel * 2.0f > (float)(iFriendlyNum + 1))
            {
               if (g_b_DebugTasks)
   ...
   if ((g_bBombPlanted) && (((pBot->bot_team == TEAM_CS_TERRORIST)
	        && (gpGlobals->time > g_fTimeBombPlanted + g_f_cv_c4timer - 15.0f))
              || (((pEdict->v.origin - g_vecBomb).Length() > 80.0f) && (pBot->bot_team == TEAM_CS_COUNTER) && (!g_bBombDefusing)
                 && (((!pBot->b_has_defuse_kit) && (gpGlobals->time > g_fTimeBombPlanted + g_f_cv_c4timer - 15.0f))
              || (gpGlobals->time > g_fTimeBombPlanted + g_f_cv_c4timer - 7.0f)))))  // KWo - 16.09.2008
   {
...
//         taskFilters[TASK_MOVETOPOSITION].fDesire = TASKPRI_HIDE;
   ...
   {
      if (/* (fabs(pBot->pBotEnemy->v.origin.z - pEdict->v.origin.z) > 45.0f)
         && ((pBot->pBotEnemy->v.origin - pBot->pEdict->v.origin).Length2D() < 256.0f) */
         ((pBot->pBotEnemy->v.origin - pBot->pEdict->v.origin).Length2D() > 100.0f)
         && (pBot->iStates & STATE_SEEINGENEMY))
      {
         ...
         {
            if (fabs(paths[iTempWp]->origin.z - pBot->pBotEnemy->v.origin.z) < 15.0f)
            {
               TempTask = taskFilters[TASK_MOVETOPOSITION];
               TempTask.fDesire = TASKPRI_HIDE;
   ...
   if ((pBot->pBotPickupItem) && (pBot->iPickupType != PICKUP_NONE)) // 16.09.2006
   {
      ...
      if (((pBot->iPickupType == PICKUP_PLANTED_C4) && (pBot->bot_team == TEAM_CS_COUNTER))
         ...
         fDistance = fDistance * 0.4f;
      else
         fDistance = fDistance * 0.2f;

      if (fDistance > 35.0f) // KWo - 25.06.2008
      {
         if (pBot->iPickupType == PICKUP_WEAPON)
         {
            ...
            else if (((pBot->f_shoot_time + 3.0f > gpGlobals->time) || (pBot->f_bot_see_enemy_time + 3.0f > gpGlobals->time))
   ...
   else
   {
      ...
      taskFilters[TASK_PICKUPITEM].fDesire = 0.0f;
   }
   ...
   {
      fDistance = (pBot->vecLastEnemyOrigin - pEdict->v.origin).Length ();
      if (pBot->pEdict->v.health > 70.0f)
         fRetreatLevel = 0.0f;
      else
         fRetreatLevel = (70.0f - pBot->pEdict->v.health) * fTempFear;
      if ((pBot->bot_team == TEAM_CS_COUNTER) && (g_bBombPlanted)) // KWo - 07.10.2010
         fRetreatLevel = 0.0f;

      fTimeSeen = pBot->f_bot_see_enemy_time - gpGlobals->time;

      if ((pBot->vecLastEnemyOrigin - pEdict->v.origin).Length() < 600.0f)
      ...
      {
         fTimeSeen += 10.0f;
         fRatio = fTimeSeen * 0.1f;
      }
      else
      {
         fTimeHeard += 10.0f;
         fRatio = fTimeHeard * 0.1f;
      }
      ...
      if ((((pBot->current_weapon.iId == CS_WEAPON_KNIFE) && (iEnWepID != CS_WEAPON_KNIFE)
            && (pBot->vecLastEnemyOrigin.z - pBot->pEdict->v.origin.z > 45.0f) && (!pBot->bOnLadder)
            && ((pBot->vecLastEnemyOrigin - pBot->pEdict->v.origin).Length2D() < 200.0f)
            && (!pBot->bIsChickenOrZombie) && (!g_b_cv_jasonmode))
          || (pBot->bIsReloading)
          || ((WeaponIsSniper(iEnWepID)) && (GetShootingConeDeviation (pBot->pLastEnemy, &pEdict->v.origin) > 0.95f)))
          && (pBot->f_bot_see_enemy_time + 3.0f > gpGlobals->time)) // KWo - 18.03.2010
      {
      ...
      if ((((pBot->bot_personality != 2)
         ...
         && ((g_fTimeRoundMid < gpGlobals->time)
             || ((((pBot->bot_team == TEAM_CS_TERRORIST) && (g_iMapType & MAP_DE))
                ...
                && (pBot->f_spawn_time + 10.0f < gpGlobals->time))
      ...
      {
         fLevel = 4096.0f - ((1.0f - fTempAgression) * fDistance);
         fLevel = (100.0f * fLevel) / 4096.0f;
      ...
      {
         taskFilters[TASK_ENEMYHUNT].fDesire = 0.0f;
   ...
   {
      taskFilters[TASK_SEEKCOVER].fDesire = 0.0f;
      taskFilters[TASK_ENEMYHUNT].fDesire = 0.0f;
   }
   ...
   {
      taskFilters[TASK_BLINDED].fDesire = 0.0f;
      pBot->bShootLastPosition = FALSE;
      pBot->bMadShoot = FALSE;
   }
   ...
   pBot->oldcombatdesire = hysteresisdesire (taskFilters[TASK_ATTACK].fDesire, 40.0f, 90.0f, pBot->oldcombatdesire);
   ...
   ptaskSurvive = thresholddesire (&taskFilters[TASK_SEEKCOVER], 40.0f, 0.0f);
   ptaskSurvive = subsumedesire (&taskFilters[TASK_HIDE], ptaskSurvive);

   // Don't allow hunting if Desire's 60<
   pDefault = thresholddesire (&taskFilters[TASK_ENEMYHUNT], 41.0f, 0.0f); // KWo - 09.04.2010
...
// KWo - at the beginning need to clean up all NULL tasks...
   ...
   {
      if (pBot->fNoCollTime + 1.0f < gpGlobals->time) // KWo - 02.01.2010
         pBot->fNoCollTime = gpGlobals->time + 1.0f;
...
inline void BotFacePosition (bot_t *pBot, Vector vecPos) // KWo - 13.04.2010
{
   ...
   {
      vecDirection = vecDirection + pEdict->v.punchangle * (float)(pBot->bot_skill)/100.0f;
...
//   vecDirection = vecDirection  + 1.0f * (pEdict->v.punchangle) * (float)(pBot->bot_skill)/100.0f; // KWo - 02.04.2010
   ...
   if (pEdict->v.idealpitch > 89.0f)
      pEdict->v.idealpitch = 89.0f;
   else if (pEdict->v.idealpitch < -89.0f)
      pEdict->v.idealpitch = -89.0f;
...
//         BotTurnSpeeds[pBot->bot_skill / 20].fMaxTurnSpeed);
      ...
      // Get Speed Multiply Factor by dividing Target FPS by real FPS
         ...
         if (fSpeedFactor < 1.0f)
            fSpeedFactor = 1.0f;

      // Slow turning if ZOOMED!
         float fZoomFactor = pEdict->v.fov * (1.f / 90.f);

         BotChangePitch (pBot, pEdict->v.pitch_speed * fSpeedFactor * fZoomFactor * (((pBot->iAimFlags & AIM_ENEMY) && !(pBot->iAimFlags & AIM_ENTITY)) ? 1.0f : 0.3f)); // KWo - 04.03.2004
         BotChangeYaw (pBot, pEdict->v.yaw_speed * fSpeedFactor * fZoomFactor * (((pBot->iAimFlags & AIM_ENEMY) && !(pBot->iAimFlags & AIM_ENTITY)) ? 1.0f : 0.3f)); // KWo - 04.03.2004
         break;
      }
      case 3:
      {
         ...
         float flTurnSkill = (pBot->bot_skill * 0.05f) + 0.5f;
         float flSpeed = (0.5f - (1.f / 3.f)) + flTurnSkill * (2.0f / 33.0f);
...
/*         if (pBot->iAimFlags & AIM_ENEMY)
         ...
         float flMomentum = (1 - flSpeed) * 0.5f;//exp (log (flSpeed * 0.5f) * flFrameCompensation);
      ...
      default:
      {
         ...
         spring_stiffness.z = 0.0f;
         damper_coefficient.x = g_f_cv_aim_damper_coefficient_x;
         damper_coefficient.y = g_f_cv_aim_damper_coefficient_y;
         damper_coefficient.z = 0.0f;
         influence.x = g_f_cv_aim_influence_x_on_y * (100 - pBot->bot_skill)/100.0f;
         influence.y = g_f_cv_aim_influence_y_on_x * (100 - pBot->bot_skill)/100.0f;
         influence.z = 0.0f;
         randomization.x = g_f_cv_aim_deviation_x * (100 - pBot->bot_skill)/100.0f;
         randomization.y = g_f_cv_aim_deviation_y * (100 - pBot->bot_skill)/100.0f;
         randomization.z = 0.0f;
         ...
         fDistStiff = 1.0f; // KWo - 08.07.2008
         ...
         if (pBot->iAimFlags & (AIM_ENEMY | AIM_LASTENEMY)) // KWo - 12.03.2010
         {
            ...
            if (pBot->iAimFlags & AIM_ENEMY)
            {
               if (!FNullEnt (pBot->pLastEnemy))
               {
                  ...
                  if (fDistStiff > 1.5f)
                     fDistStiff = 1.5f;
                  else if (fDistStiff < 1.0f)
                     fDistStiff = 1.0f;

//                  pBot->target_angular_speed = (UTIL_VecToAngles (pBot->vecLastEnemyOrigin - pEdict->v.origin
//                     + 1.0f * pBot->fTimeFrameInterval * pBot->pLastEnemy->v.velocity + 1.0f * (pBot->fTimeFrameInterval) * pEdict->v.velocity)
//                     - UTIL_VecToAngles (pBot->vecLastEnemyOrigin - pEdict->v.origin)) * 0.45 * ((float)pBot->bot_skill/100.0f) * target_anticipation_ratio;

                  pBot->target_angular_speed = (UTIL_VecToAngles (pBot->vecLastEnemyOrigin - pEdict->v.origin
                     + 1.0f * /* pBot->fTimeFrameInterval * */ pBot->pLastEnemy->v.velocity - 1.0f /* * (pBot->fTimeFrameInterval) */ * pEdict->v.velocity)
                     - UTIL_VecToAngles (pBot->vecLastEnemyOrigin - pEdict->v.origin)) * 0.45 * ((float)pBot->bot_skill/100.0f) * target_anticipation_ratio; // KWo - 12.01.2012


                  pBot->target_angular_speed.x = -pBot->target_angular_speed.x;

                  if ((pEdict->v.fov < 90) /* && (pBot->angular_deviation.Length () >= 5.0f) */)
                  {
...
//                  UTIL_ClampVector (&pBot->target_angular_speed);
               ...
               v_stiffness = fDistStiff * spring_stiffness * (0.2 + ((float) pBot->bot_skill) / 125.0f);
               if (pEdict->v.punchangle.x > 2.0f)
                  v_stiffness.x = v_stiffness.x * 2;
            }
            else
            {
               pBot->target_angular_speed = g_vecZero;
               v_stiffness = 0.4 * spring_stiffness * (0.2 + ((float) pBot->bot_skill) / 125.0f);
            }
         }
         else
         {
            ...
            if ((pBot->randomize_angles_time < gpGlobals->time)
                && (((pBot->pEdict->v.velocity.Length () > 1.0f) && (pBot->angular_deviation.Length () < 5.0f))
                    || (pBot->angular_deviation.Length () < 1.0f)))
            {
               // is the bot standing still ?
               if (pBot->pEdict->v.velocity.Length () < 1.0f)
                  vecRandomization = randomization * 0.2f; // randomize less
               else
                  vecRandomization = randomization;

               // randomize targeted location a bit (slightly towards the ground)
               pBot->randomized_angles = Vector (RANDOM_FLOAT (-vecRandomization.x * 0.5f, vecRandomization.x * 1.5f),
                                                                          RANDOM_FLOAT (-vecRandomization.y, vecRandomization.y), 0.0f);
               // set next time to do this
               pBot->randomize_angles_time = gpGlobals->time + RANDOM_FLOAT (0.4f, offset_delay);
            }

            if ((((pBot->dest_origin - pBot->pEdict->v.origin).Length() <= 15.0f) && (pBot->bMoveToGoal)
            ...
            if (gpGlobals->time - (pBot->player_target_time + offset_delay) < notarget_slowdown_ratio * 10.0f)
               stiffness_multiplier = 1 - (gpGlobals->time - pBot->fTimeLastFired) * 0.1;

            if (stiffness_multiplier < 0.1f) // KWo - 17.02.2008
               stiffness_multiplier = 0.1f;
// \\ KWo - TEST against bot "shaking"
            if ((pBot->f_wpt_timeset + 1.0f > gpGlobals->time) && pBot->bMoveToGoal) // KWo - 20.09.2008
               stiffness_multiplier = 1.5f * stiffness_multiplier;

            // also take in account the remaining deviation (slow down the aiming in the last 10°)

            if (pBot->angular_deviation.Length () < 10.0f)
            {
            ...
            if (pBot->pEdict->v.velocity.Length () < 1.0f)
               stiffness_multiplier *= 0.5f;


            // but don't allow getting below a certain value
            if (stiffness_multiplier < 0.2f)
               stiffness_multiplier = 0.2f;
...
//         pBot->angular_deviation = pBot->randomized_ideal_angles + pBot->target_angular_speed - pBot->pEdict->v.v_angle;
         ...
         pBot->aim_speed.x = (v_stiffness.x * pBot->angular_deviation.x)
             - (damper_coefficient.x * (pBot->aim_speed.x)) + 1.0f * pBot->target_angular_speed.x;  // KWo - 12.01.2012
         pBot->aim_speed.y = (v_stiffness.y * pBot->angular_deviation.y)
             - (damper_coefficient.y * (pBot->aim_speed.y)) + 1.0f * pBot->target_angular_speed.y;  // KWo - 12.01.2012
         ...
         {
            if (pBot->aim_speed.x > 50.0f)
               pBot->aim_speed.x = 50.0f;
            else if (pBot->aim_speed.x < -50.0f)
               pBot->aim_speed.x = -50.0f;

            if (pBot->aim_speed.y > 200.0f)
               pBot->aim_speed.y = 200.0f;
            else if (pBot->aim_speed.y < -200.0f)
               pBot->aim_speed.y = -200.0f;
         }

         // move the aim cursor
         pBot->pEdict->v.v_angle = pBot->pEdict->v.v_angle + pBot->fTimeFrameInterval * Vector (pBot->aim_speed.x, pBot->aim_speed.y, 0.0f); // KWo - 17.10.2006
         if (pBot->pEdict->v.v_angle.x < -89.0f) // KWo - 10.04.2016
            pBot->pEdict->v.v_angle.x = -89.0f;
         else if (pBot->pEdict->v.v_angle.x > 89.0f) // KWo - 10.04.2016
            pBot->pEdict->v.v_angle.x = 89.0f;
      ...
      if (pBot->fEnemyReachableTimer < gpGlobals->time)
      {
         ...
         if  ((iPathDist - iLinDist > 112)
            || ((pBot->current_weapon.iId == CS_WEAPON_KNIFE) && (iEnWepID != CS_WEAPON_KNIFE)
                && (pBot->pBotEnemy->v.origin.z - pBot->pEdict->v.origin.z > 45.0f)
                && ((pBot->pBotEnemy->v.origin - pBot->pEdict->v.origin).Length2D() < 100.0f)
         ...
         pBot->fEnemyReachableTimer = gpGlobals->time + 1.0f;
      }
   ...
   switch (pBot->iRadioOrder)
   {
      ...
      case RADIO_HOLDPOSITION:
      {
         if (!FNullEnt (pBot->pBotUser) && (pBot->pBotUser == pPlayer))
         {
            ...
            pBot->iCampButtons = 0;
            bottask_t TempTask = {NULL, NULL, TASK_PAUSE, TASKPRI_PAUSE, -1, gpGlobals->time + RANDOM_FLOAT (30.0f, 60.0f), FALSE};
            BotPushTask (pBot, &TempTask);
         }
         break;
      }
      ...
      case RADIO_TAKINGFIRE:
      {
         if (FNullEnt (pBot->pBotUser))
         {
            if (FNullEnt (pBot->pBotEnemy))
            {
               ...
               bottask_t TempTask = {NULL, NULL, TASK_MOVETOPOSITION, TASKPRI_MOVETOPOSITION, iIndex, 0.0f, TRUE}; // KWo - 28.08.2006
               BotPushTask (pBot, &TempTask);
            }
            else if (g_b_cv_radio) // KWo - 07.03.2010
               BotPlayRadioMessage (pBot, RADIO_NEGATIVE);
...
// KWo - maybe instead this we should find the closest door and the bots should storm these doors? only if there is no door - random stuff?
      ...
      case RADIO_FALLBACK:
      {
         if ((FNullEnt (pBot->pBotEnemy) && (BotEntityIsVisible (pBot, pPlayer->v.origin)))
             || (f_distance < 1024.0f))
         {
            pBot->fFearLevel += 0.5f;
            if (pBot->fFearLevel > 1.0f)
               pBot->fFearLevel = 1.0f;

            pBot->fAgressionLevel -= 0.5f;
            if (pBot->fAgressionLevel < 0.0f)
               pBot->fAgressionLevel = 0.0f;
            ...
            if (BotGetSafeTask(pBot)->iTask == TASK_CAMP)
            {
               BotGetSafeTask(pBot)->fTime += RANDOM_FLOAT (10.0f, 15.0f);
...
void BotChooseAimDirection (bot_t *pBot)
{
   ...
   if ((pBot->vecLastEnemyOrigin != g_vecZero) && (!FNullEnt(pBot->pLastEnemy)))          // KWo - 10.07.2008
   {
      ...
      if (FNullEnt(pBot->pBotEnemy) && (pBot->f_bot_see_enemy_time + 2.0f < gpGlobals->time)
         && (((tr.flFraction <= 0.2f) && (FStrEq(STRING (tr.pHit->v.classname), "worldspawn"))
          && (fLastEnDistance >= 300.0f)
/*          && !(pBot->bShootThruSeen) && !(pBot->bShootThruHeard) */
          && (pBot->f_bot_see_new_enemy_time - 0.5f < gpGlobals->time))
          || (!(pBot->bShootThruSeen) && (fabs(vLastEnLookDir.x) >= 45.0f))))              // KWo - 01.10.2010
      {
         ...
         if ((pBot->fChangeAimDirectionTime + 0.5f < gpGlobals->time) && !(pBot->bShootThruSeen) && !(pBot->bShootThruHeard))
         {
            pBot->iAimFlags &= ~AIM_LASTENEMY;
   ...
   if ((BotGetSafeTask(pBot)->iTask != TASK_DEFUSEBOMB) && (pBot->pShootBreakable == NULL) && (pBot->fButtonNoticedTime + 5.0f < gpGlobals->time)
      ...
      && (BotGetSafeTask(pBot)->iTask != TASK_PICKUPITEM))  // KWo - 27.10.2006
   {
      if (bCurWptIndOK)
      {
         if ((!(paths[pBot->curr_wpt_index]->flags & W_FL_USE_BUTTON)) && (pBot->fButtonNoticedTime + 3.0f < gpGlobals->time)) // KWo - 29.03.2008
         {
   ...
   if (!(iFlags & (AIM_GRENADE | AIM_ENEMY | AIM_ENTITY)))
   {
      if ((pBot->bOnLadder || pBot->bInWater
         || (pBot->curr_travel_flags & C_FL_JUMP) || (pBot->iWPTFlags & W_FL_LADDER)) && (fLastEnDistance > 500.0f)) // KWo - 19.01.2008
      {
   ...
   if (iFlags & AIM_OVERRIDE)
   {
      ...
      pBot->fChangeAimDirectionTime = gpGlobals->time + 1.0f;      // KWo - 12.03.2010
   }
   else if (iFlags & AIM_GRENADE)
   {
      pBot->vecLookAt = pBot->vecThrow; // KWo - 19.06.2006
      f_z_corr = 0.0f; // KWo - 16.09.2006
      f_angle_corr = 0.0f; // KWo - 16.09.2006
      f_dist_throw = (pBot->vecThrow - pEdict->v.origin).Length(); // KWo - 16.09.2006
      if ((f_dist_throw > 100.0f) && (f_dist_throw < 800.0f)) // KWo - 25.02.2008
      {
         f_angle_corr = 0.0f;
         f_z_corr = (0.25f) * (pBot->vecThrow.z - pEdict->v.origin.z);  // KWo - 25.02.2008
      }
      else if (f_dist_throw >= 800.0f)
      {
         f_angle_corr = 37.0f * (f_dist_throw - 800.0f)/800.0f;
         if (f_angle_corr > 45.0f)
            f_angle_corr = 45.0f;
         f_z_corr = f_dist_throw * tan (f_angle_corr * M_PI / 180.0f)
   ...
   else if (iFlags & AIM_ENEMY) // KWo - 04.03.2006
   {
      ...
      BotFocusEnemy(pBot);             // KWo - 10.10.2006
      pBot->vecLookAt = pBot->vecEnemy; // KWo - 10.10.2006
      pBot->fChangeAimDirectionTime = gpGlobals->time + 1.0f; // KWo - 06.02.2008
   }
   else if (iFlags & AIM_LASTENEMY)// KWo - 04.03.2006
   {
      // Did Bot just see Enemy and is quite agressive ?
      if ((pBot->f_heard_sound_time + 4.0f > gpGlobals->time)) // KWo - 12.07.2008
      {
   ...
   else if (iFlags & AIM_PREDICTPATH) // KWo - 04.03.2006
   {
      if  ( (((pEdict->v.origin - pBot->vecLastEnemyOrigin).Length () < 1600) || BotUsesSniper(pBot))
          && ( (tr.flFraction >= 0.2f) || !(FStrEq(STRING (tr.pHit->v.classname), "worldspawn")))) // KWo - 22.10.2006
      {
         bRecalcPath = TRUE;
         ...
         if ((paths[pBot->curr_wpt_index]->flags & W_FL_LADDER) && (pBot->pWaypointNodes != NULL))   // KWo - 22.01.2012
         {
            if (pBot->pWaypointNodes->NextNode != NULL)
            {
               if ((paths[pBot->pWaypointNodes->NextNode->iIndex]->flags & W_FL_LADDER) && ((pEdict->v.origin - pBot->dest_origin).Length () < 100)
                  && (paths[pBot->pWaypointNodes->NextNode->iIndex]->origin.z > paths[pBot->curr_wpt_index]->origin.z + 30.0f))
               {
...
//    look backward if the bot is looking at the wall...
               ...
               && (pBot->cCollideMoves[(int) pBot->cCollStateIndex] != COLLISION_STRAFERIGHT))))   // KWo - 16.12.2007
      {
         if (!(paths[pBot->prev_wpt_index[0]]->flags & W_FL_LADDER)
              ...
              && ((!(pBot->iWPTFlags & W_FL_CROUCH) && (fabs(paths[pBot->prev_wpt_index[0]]->origin.z - pBot->dest_origin.z) < 30.0f))
                  || (pBot->iWPTFlags & W_FL_CAMP)))// KWo - 23.03.2006
         {
            ...
            if ((tr.flFraction < 1.0f) && (FStrEq("worldspawn", STRING (tr.pHit->v.classname))))
            {
...
//    look at the danger position if it's seen from bot's eyes...
      if (pBot->bCanChooseAimDirection && (pBot->f_timeHitDoor + 2.0f < gpGlobals->time)
         ...
         if (!(paths[pBot->prev_wpt_index[0]]->flags & W_FL_LADDER)) // KWo - 12.08.2006
         {
            if (pBot->bot_team == TEAM_CS_TERRORIST)
            {
                  ...
                  && ((float)((pBotExperienceData + (iIndex * g_iNumWaypoints) + iIndex)->uTeam0Damage)/g_iHighestDamageT > 0.6)) // KWo - 07.04.2010
               {
                  ...
                  if ((tr.flFraction > 0.8f) || (!FStrEq("worldspawn", STRING (tr.pHit->v.classname))))  // KWo - 12.03.2006
                  {
            ...
            else
            {
               if (((pBotExperienceData + (iIndex * g_iNumWaypoints) + iIndex)->iTeam1_danger_index != -1)
                  && ((float)((pBotExperienceData + (iIndex * g_iNumWaypoints) + iIndex)->uTeam1Damage)/g_iHighestDamageCT > 0.6f)) // KWo - 07.04.2010
               {
      ...
      {
         if ((pBot->dest_origin - pBot->pEdict->v.origin).Length() > 15.0f)
            pBot->vecLookAt = pBot->dest_origin + pBot->pEdict->v.view_ofs;
...
void BotCheckShield (bot_t *pBot) // KWo - 13.04.2010
{
   ...
   if (pBot->pBotEnemy)
   {
      ...
      if (BotHasShield(pBot)) // KWo - 17.07.2007
      {
         ...
         {
            if ((WeaponIsPistol(iWeaponEnemy) || WeaponIsPrimaryGun(iWeaponEnemy)) && (fEnemyDot > 0.92f)
               && (pBot->pBotEnemy->v.button & IN_ATTACK) && (pBot->fTimeWeaponSwitch + 0.5f < gpGlobals->time)
         ...
         {
            if (((!WeaponIsPistol(iWeaponEnemy) && !WeaponIsPrimaryGun(iWeaponEnemy)) || (fEnemyDot < 0.85f)
               || (clients[iEnemyIndex].fReloadingTime >= gpGlobals->time)) // KWo - 15.08.2007)
                 && (pBot->fTimeWeaponSwitch + 0.2f < gpGlobals->time))
            {
               pBot->pEdict->v.button |= IN_ATTACK2;
               pBot->fTimeWeaponSwitch = gpGlobals->time;
            }
...
void BotCheckDarkness (bot_t *pBot)  // KWo - 13.04.2010
{
   ...
   if ((g_i_botthink_index == g_iFrameCounter)
        && (pBot->f_spawn_time + 6.0f < gpGlobals->time)) // KWo - 23.03.2012
   {
      ...
      {
         if ((((g_f_cv_skycolor > 50.0f) && (fLightLevel < 10.0f)) || ((g_f_cv_skycolor <= 50.0f) && (fLightLevel < 40.0f)))
            && (!(pEdict->v.effects & EF_DIMLIGHT)) && (BotGetSafeTask(pBot)->iTask != TASK_CAMP)
            && (BotGetSafeTask(pBot)->iTask != TASK_ATTACK) && (pBot->f_heard_sound_time + 3.0f < gpGlobals->time)
            && (pBot->cFlashBat > 30))
         {
            pEdict->v.impulse = 100;
         }
         else if ((pEdict->v.effects & EF_DIMLIGHT)
               && ((((fLightLevel > 15.0f) && (g_f_cv_skycolor > 50.0f)) || ((fLightLevel > 45.0f) && (g_f_cv_skycolor <= 50.0f)))
               || (BotGetSafeTask(pBot)->iTask == TASK_CAMP) || (BotGetSafeTask(pBot)->iTask == TASK_ATTACK)
               || (pBot->cFlashBat <= 0) || (pBot->f_heard_sound_time + 3.0f >= gpGlobals->time)))
         {
            pEdict->v.impulse = 100;
         }
      }
      else if (bNvg && gmsgNVGToggle)
      {
         ...
         else if ((((g_f_cv_skycolor > 50.0f) && (fLightLevel < 15.0f)) || ((g_f_cv_skycolor <= 50.0f) && (fLightLevel < 40.0f)))
            && !pBot->bUsesNVG)
         {
         ...
         else if ((((fLightLevel > 20.0f) && (g_f_cv_skycolor > 50.0f)) || ((fLightLevel > 45.0f) && (g_f_cv_skycolor <= 50.0f)))
               && pBot->bUsesNVG)
         {
            FakeClientCommand (pEdict, "nightvision\n");
...
void BotExecuteTask (bot_t *pBot)
{
   ...
   pEdict = pBot->pEdict;
   bCampingAllowed = FALSE; // KWo - 29.07.2018
      ...
      // calculate new path to Bomb Point if he's not already heading for
         if (g_bBombPlanted && (pBot->bot_team == TEAM_CS_COUNTER) && (g_iDebugGoalIndex == -1) )  // KWo - 03.02.2006
         {
            if (BotGetSafeTask(pBot)->iData != -1)
            {
               ...
               && ((paths[BotGetSafeTask(pBot)->iData]->origin - g_vecBomb).Length() > 128.0f)) // KWo - 16.05.2008
               {
                  DeleteSearchNodes (pBot);
                  BotGetSafeTask(pBot)->iData = -1;
               }
         ...
         if ((pBot->curr_wpt_index == BotGetSafeTask(pBot)->iData) && (BotGetSafeTask(pBot)->iData != -1)) // KWo - 15.06.2008
         {
            ...
            if (paths[pBot->curr_wpt_index]->flags & W_FL_CAMP)
            {
               ...
               if (BotHasCampWeapon (pBot) && (pBot->current_weapon.iAmmo1 > 0) && (pBot->fTimeCamping + 10.0f < gpGlobals->time)
                && !((pBot->bot_team == TEAM_CS_COUNTER) && g_bBombPlanted)) // KWo - 25.05.2010
               {
                  ...
                  for (c = 0; c < gpGlobals->maxClients; c++)
                  {
                     ...
                     if (!FNullEnt(pPlayer))
                     {
                        ...
                        if (((pPlayer->v.origin - paths[pBot->curr_wpt_index]->origin).Length() < 50.0f)
                           && ((pPlayer->v.velocity).Length2D() < 30.0f))
                           bCampingAllowed = FALSE;
                     }
                  }
                  if (!bCampingAllowed)
            ...
            if (paths[pBot->curr_wpt_index]->flags & W_FL_CAMP)
            {
                ...
                && ((pBot->bot_team != TEAM_CS_COUNTER) || (!g_bBombPlanted)) && (!pBot->bIsChickenOrZombie)) // KWo - 18.01.2011
               {
                  ...
                  if (bCampingAllowed)
                  {
                     ...
                     pBot->fTimeCamping = RANDOM_FLOAT (15.0f, 30.0f); // KWo - 23.03.2008
                     ...
                     MAKE_VECTORS (Vector (paths[pBot->curr_wpt_index]->fcampstartx, paths[pBot->curr_wpt_index]->fcampstarty, 0.0f));
            ...
            else
            {
               ...
               if (g_iMapType & MAP_CS)
               {
                  ...
                  if (BotHasHostage (pBot) && (pBot->bot_team == TEAM_CS_COUNTER))
                  {
                     ...
                     {
                        if (pBot->fTimeHostageRescue == 0.0f) // KWo - 27.02.2007
                        ...
                        for (i = 0; i < MAX_HOSTAGES; i++)
                        {
                           ...
                           {
                              if ((pBot->pHostages[i]->v.takedamage == 0) || (pBot->fTimeHostageRescue + 5.0f < gpGlobals->time)) // KWo - 27.02.2007
                              {
                                 pBot->pHostages[i] = NULL;
                                 if (pBot->fTimeHostageRescue + 5.0f > gpGlobals->time)
               ...
               if (g_iMapType & MAP_DE)
               {
                  ...
                  if (paths[pBot->curr_wpt_index]->flags & W_FL_GOAL)
                  {
                        ...
                        && (pEdict->v.weapons & (1 << CS_WEAPON_C4)))  // KWo - 25.04.2008
                     {
                        ...
                        bottask_t TempTask = {NULL, NULL, TASK_PLANTBOMB, TASKPRI_PLANTBOMB, -1, 0.0f, TRUE};   // KWo - 21.02.2007
                        BotPushTask (pBot, &TempTask);
                     ...
                     else if ((pBot->bot_team == TEAM_CS_COUNTER) && g_bBombPlanted)
                     {
                        ...
                        if (distance < 150)  // KWo - 05.09.2008
                        {
                           ...
                           if ((iTempWpIndex1 >= 0) && (iTempWpIndex1 < g_iNumWaypoints) && (iTempWpIndex1 != pBot->curr_wpt_index))
                           {
                              ...
                              bottask_t TempTask = {NULL, NULL, TASK_MOVETOPOSITION, TASKPRI_MOVETOPOSITION + 11.0f, iTempWpIndex1, 0.0f, TRUE}; // KWo - 02.09.2008
                              BotPushTask (pBot, &TempTask);
                           }
                        }
...
//          && (BotGetSafeTask(pBot)->iTask != TASK_PLANTBOMB)) // KWo - 12.08.2006
         {
            ...
            pBot->fNoCollTime = gpGlobals->time + 1.0f;   // KWo - 02.01.2010

            if ((!g_bPathWasCalculated) && ((g_i_botthink_index == g_iFrameCounter) || (pBot->f_spawn_time + 10.0f < gpGlobals->time))) // KWo - 18.02.2008
            {
               DeleteSearchNodes (pBot);
...
//                  pBot->chosengoal_index = iDestIndex;  // KWo - 21.04.2006
            ...
            else
            {
               ...
               if (!(pEdict->v.flags & FL_DUCKING)
                  && (pBot->fMinSpeed != 60.0f))
                  pBot->f_move_speed = 60.0f;
            }
         }
      ...
      // checks once a while if any bomb site isn't so close so we can just plant the bomb instead go somewhere else...
            ...
            if (min_index > -1)
            {
               ...
               if (!(paths[pBot->chosengoal_index]->flags & W_FL_GOAL)
                  || ((paths[pBot->chosengoal_index]->origin - paths[iGoalIndex]->origin).Length() > 800.0f))
               {
                  if (g_b_DebugTasks || g_b_DebugNavig)
            ...
            else if ((paths[pBot->chosengoal_index]->origin - pEdict->v.origin).Length() < 1000.0f) // KWo - 14.04.2008
            {
               if (IsGroupOfEnemies(pBot, paths[pBot->chosengoal_index]->origin))
               {
                  if (g_b_DebugTasks || g_b_DebugNavig)
         ...
         // checks once a while if the bomb position isn't so close so we can see it and go there instead to go somewhere else...
            if (((paths[pBot->chosengoal_index]->origin - g_vecBomb).Length() > 128.0f)
               || (fabs(paths[pBot->chosengoal_index]->origin.z - g_vecBomb.z) > 40.0f)) // KWo - 05.09.2008
            {
               ...
               if (tr.flFraction >= 0.8)
               {
                  ...
                  for (i = 0; i < g_iNumGoalPoints; i++)
                  {
                     ...
                     if ((distance < min_distance) && (fabs(paths[iGoalIndex]->origin.z - g_vecBomb.z) < 40.0f)) // KWo - 14.08.2008
                     {
                        min_index = i;
                        min_distance = distance;
                     }
                  ...
                  if (iGoalIndex > -1)
                  {
                     DeleteSearchNodes (pBot);
                     pBot->chosengoal_index = iGoalIndex;
                     bottask_t TempTask = {NULL, NULL, TASK_MOVETOPOSITION, TASKPRI_MOVETOPOSITION + 11.0f, iGoalIndex, 0.0f, TRUE}; // KWo - 02.09.2008
                     BotPushTask (pBot, &TempTask);
            ...
            && (pBot->f_shoot_time - 0.4 <= gpGlobals->time) && (pBot->iStates & STATE_SEEINGENEMY)) // KWo - 22.10.2006
         {
            ...
            if (flDot1 > 0.95)
            {
               pBot->f_move_speed = 0.0f;
               pBot->f_sidemove_speed = 0.0f;
               pBot->prev_time = gpGlobals->time + 2.0f; // KWo - 25.02.2008
               pBot->fNoCollTime = gpGlobals->time + 2.5f;   // KWo - 02.01.2010
               pBot->f_wpt_timeset = gpGlobals->time + 3.0f;
            }
         }
         break;
      }
      ...
      case TASK_SPRAYLOGO:
      {
         ...
         if (!pBot->bLogoSprayed && (BotGetSafeTask(pBot)->fTime > gpGlobals->time))
         {
            ...
            // No Wall in Front ?
            if (tr.flFraction >= 1.0f)
               vecSprayPos.z -= 128.0f;
            pBot->vecEntity = vecSprayPos;

            if (BotGetSafeTask(pBot)->fTime - 0.5f < gpGlobals->time)
            {
               // Emit Spraycan sound
      ...
      case TASK_ENEMYHUNT:
      {
         ...
         // seen the new enemy
         if (((!FNullEnt (pBot->pHuntEnemy) && (pBot->f_bot_see_enemy_time + 0.5f > gpGlobals->time))
         ...
         else if (!FNullEnt (pBot->pHuntEnemy)) // KWo - 05.08.2006
         {
            ...
            {
               if ((paths[iGoalIndex]->origin - pBot->vecHuntEnemyOrigin).Length() > 300.0f)
               {
                  if (pBot->iStates & STATE_SEEINGENEMY)
                     bNewPosition = true;
                  else
            ...
            if ((!IsAlive(pBot->pHuntEnemy)) || ((pEdict->v.origin - pBot->vecHuntEnemyOrigin).Length() < 114.0f)
         ...
         if ((pBot->bot_skill > 60) && (!FNullEnt (pBot->pHuntEnemy)) && (!bEndTask))
         {
            if ((pBot->bot_skill > 80) && (pBot->bIsChickenOrZombie))
            {
                  ...
                  && (pBot->f_ducktime < gpGlobals->time))
               {
                  ...
                  if ((RANDOM_LONG (1, 1000) < 200) && (pEdict->v.velocity.Length2D () > 100.0f)
                     && (fEnemyDot > 0.95f) && (pBot->iStates & STATE_SEEINGENEMY))
                  {
            ...
            if (!(pBot->curr_travel_flags & C_FL_JUMP) && (pBot->f_jumptime + 2.0f < gpGlobals->time))
            {
               if ((pBot->curr_wpt_index >= 0) && (pBot->curr_wpt_index < g_iNumWaypoints))
               {
                     ...
                     && (pBot->bot_skill < 80))
                  {
                     ...
                     pBot->f_ducktime = gpGlobals->time + 1.0f;
                  }
               }
               ...
               if ((v_diff.Length () < 512.f) && !(pEdict->v.flags & FL_DUCKING))
                  pBot->f_move_speed = pEdict->v.maxspeed * 0.5f;
            }
         }
         ...
         // Reached final Cover waypoint ?
            if (BotDoWaypointNav (pBot))
            {
               ...
               if (BotGetSafeTask(pBot)->iTask == TASK_SEEKCOVER) // KWo - 02.09.2006 - against BotRemoveCertainTask
               {
                  pBot->prev_goal_index = BotGetSafeTask(pBot)->iData; // KWo - 18.01.2008
                  BotTaskComplete(pBot);
                  bottask_t TempTask = {NULL, NULL, TASK_HIDE, TASKPRI_HIDE, -1, gpGlobals->time + RANDOM_FLOAT (4.0f, 10.0f), FALSE};
                  BotPushTask (pBot, &TempTask);
...
*/
                  ...
                  if (paths[pBot->curr_wpt_index]->flags & W_FL_CAMP)
                  {
                     ...
                     bottask_t TempTask1 = {NULL, NULL, TASK_CAMP, TASKPRI_CAMP, -1, gpGlobals->time + RANDOM_FLOAT (15.0f, ((20.0f > g_f_cv_maxcamptime) ? g_f_cv_maxcamptime : 20)), FALSE}; // KWo - 18.01.2008
                     BotPushTask (pBot, &TempTask1); // KWo - 18.01.2008
                  }
                  else
                  {
      ...
      case TASK_ATTACK:
      {
         ...
         if (!FNullEnt (pBot->pBotEnemy) && (pBot->f_bot_see_enemy_time + 1.0f > gpGlobals->time))
         {
            ...
            if (pBot->cCollisionState == COLLISION_NOTDECIDED) // KWo - 20.05.2008
            {
               ...
               {
                  pBot->prev_time = gpGlobals->time + 2.0f;
                  pBot->f_moved_distance = 15.0f;               // KWo - 25.05.2010
                  pBot->fNoCollTime = gpGlobals->time + 2.5f;   // KWo - 02.01.2010
               }
            }
...
//                pBot->dest_origin = pBot->vecLastEnemyOrigin;
      ...
      case TASK_PAUSE:
      {
         ...
         if ((pBot->f_view_distance < 500.0f) && (pBot->bot_skill > 60))
         {
            // Go mad !
            pBot->f_move_speed = -fabs ((pBot->f_view_distance - 500.0f) * 0.5f);
      ...
      case TASK_CAMP:
      {
         ...
         if ((pBot->fNextCampDirectionTime + 15.0f < gpGlobals->time)
         ...
         if (pBot->fNextCampDirectionTime < gpGlobals->time)
         {
            ...
            if (v_dest == g_vecZero)
            {
               ...
               {
//                  v_dest = Vector (0.0f, RANDOM_FLOAT (-180, 180), 0.0f); // KWo - 19.01.2012 - last chance
                  index2 = 1;
                  v_dest = GetGunPosition (pEdict) + Vector (1414.0f, 1414.0f, 0.0f); // trace the distance 2000.0f units
                  ...
                  {
                     case 1:
                        v_dest = UTIL_VecToAngles(Vector (1414.0f, 1414.0f, 0.0f)) + Vector (0.0f, RANDOM_FLOAT (-20.0f, 20.0f), 0.0f);
                     case 2:
                        v_dest = UTIL_VecToAngles(Vector (-1414.0f, 1414.0f, 0.0f)) + Vector (0.0f, RANDOM_FLOAT (-20.0f, 20.0f), 0.0f);
                     case 3:
                        v_dest = UTIL_VecToAngles(Vector (-1414.0f, -1414.0f, 0.0f)) + Vector (0.0f, RANDOM_FLOAT (-20.0f, 20.0f), 0.0f);
            ...
            pBot->fNextCampDirectionTime = gpGlobals->time + RANDOM_FLOAT (4.0f, 8.0f); // KWo - 27.02.2007
         }
...
/*
            ...
            && (pBot->f_heard_sound_time + 4.0f < gpGlobals->time))
         {
            ...
            if ((pBot->bot_skill >= 40) && (i != 0)
               ...
               && (pBot->f_bot_see_enemy_time + 4.0f < gpGlobals->time)
               && (pBot->f_heard_sound_time + 4.0f < gpGlobals->time)) // KWo - 19.03.2007
...
*/
      ...
      case TASK_PLANTBOMB:
      {
         ...
         {
            pBot->vecEntity = paths[pBot->curr_wpt_index]->origin - Vector(0.0f, 0.0f, 36.0f);
            pBot->iAimFlags |= AIM_ENTITY;
         }
         else
            pBot->iAimFlags |= AIM_DEST;
         ...
         if ((pBot->iStates & STATE_SEEINGENEMY)
             ...
             || ((pEdict->v.origin - pBot->vecLastEnemyOrigin).Length() < 60.0f))
         ...
         else
         {
            if (BotGetSafeTask(pBot)->iTask == TASK_PLANTBOMB) // KWo - 02.09.2006 - against BotRemoveCertainTask
            {
               ...
               if (!(pEdict->v.weapons & (1 << CS_WEAPON_C4)))
               {
                  ...
                  fCampTime = (g_f_cv_c4timer * 0.5f); // KWo - 23.03.2008
                  if (fCampTime > g_f_cv_maxcamptime)       // KWo - 23.03.2008
                     fCampTime = g_f_cv_maxcamptime;        // KWo - 23.03.2008
...
/*
         if ((BotUsesSniper (pBot))
            && FNullEnt (pBot->pBotEnemy)
            && (pBot->f_bot_see_enemy_time + 1.0f < gpGlobals->time))
         {
            // Try zooming out to move fast again
            i = HighestPistolOfEdict(pEdict);
            ...
            if ((pBot->bot_skill >= 40) && (i != 0)
               && (pBot->current_weapon.iId != cs_weapon_select[i].iId)
               && (pBot->current_weapon.iId != CS_WEAPON_INSWITCH)
               && (pBot->fTimeWeaponSwitch + 0.5f < gpGlobals->time)
               && (pBot->f_bot_see_enemy_time + 4.0f < gpGlobals->time)
               && (pBot->f_heard_sound_time + 4.0f < gpGlobals->time)) // KWo - 19.03.2007
                  SelectWeaponByNumber (pBot, i);
         }
*/
         ...
         if (BotFollowUser (pBot))
         {
            ...
            if ((((fDistanceToUser > 256.0f)
               || (!FVisible (pBot->pBotUser->v.origin, pEdict)) )
               && (bBotReachedDest)) || (fDistanceToUser < 50.0f)) // KWo - 26.02.2007
            {
               if (((g_i_botthink_index != g_iFrameCounter) || (g_bPathWasCalculated))
                  && (pBot->f_spawn_time + 10.0f > gpGlobals->time)) // KWo - 18.02.2008
               {
                  pBot->f_move_speed = 0.0f;
                  pBot->f_moved_distance = 15.0f;      // KWo - 25.05.2010
                  if (fDistanceToUser > 50.0f)         // KWo - 16.01.2010
                  {
                     pBot->prev_time = gpGlobals->time + 1.5f; // KWo - 25.02.2008
                     pBot->fNoCollTime = gpGlobals->time + 2.0f;   // KWo - 02.01.2010
//                pBot->f_wpt_timeset = gpGlobals->time;  // KWo - 30.07.2006
                     ...
                     pBot->fNoCollTime = gpGlobals->time - 2.0f;   // KWo - 27.03.2010
               }
               else
               {
                  iTempIndex = WaypointFindNearestToMove (pBot->pBotUser, pBot->pBotUser->v.origin); // KWo - 17.04.2008
...
//            pEdict->v.button &= ~IN_JUMP;
      ...
      case TASK_THROWHEGRENADE:
      {
         ...
         pBot->f_moved_distance = 15.0f;               // KWo - 25.05.2010
         pBot->fNoCollTime = gpGlobals->time + 2.5f;   // KWo - 02.01.2010
         pBot->f_grenade_check_time = gpGlobals->time + RANDOM_FLOAT (4.0f, 7.0f); // KWo - 24.08.2008
         ...
         if (!FNullEnt (pBot->pLastEnemy) && (pBot->vecLastEnemyOrigin != g_vecZero))
         {
            ...
            if (pBot->iStates & STATE_SEEINGENEMY)
            {
               if (IsAlive (pBot->pLastEnemy)) // KWo - 26.07.2007
               {
                  ...
                  v_src.z = 0.0f;
                  v_dest = v_dest + (v_src * distance / 400.0f);
               }
               else
                  v_dest = g_vecZero;
            }
         ...
         {
            pBot->vecGrenade = VecCheckThrow  (pBot, GetGunPosition (pEdict), v_dest, 1.0f);
            ...
            || (NumTeammatesNearPos (pBot, v_dest, 512) > 0)) // KWo - 25.08.2008
         {
            ...
            if ((pBot->fTimeWeaponSwitch + 1.0f < gpGlobals->time) && (BotGetSafeTask(pBot)->iTask == TASK_THROWHEGRENADE)) // KWo - 22.01.2008
            {
               BotSelectBestWeapon (pBot);
               BotTaskComplete(pBot);
            }
         ...
         else if ((pBot->vecGrenade != g_vecZero) /* || (pBot->current_weapon.iId == CS_WEAPON_HEGRENADE) */) // KWo - 22.01.2008 - rewritten again...
         {
            if ((pBot->current_weapon.iId != CS_WEAPON_HEGRENADE) && (pBot->current_weapon.iId != CS_WEAPON_INSWITCH)
               && (pBot->fTimeWeaponSwitch + 0.5f < gpGlobals->time) /* && !FStrEq(STRING(pEdict->v.viewmodel),"models/v_hegrenade.mdl") */)
            {
      ...
      case TASK_THROWFLASHBANG:
      {
         ...
         pBot->f_moved_distance = 15.0f;               // KWo - 25.05.2010
         pBot->fNoCollTime = gpGlobals->time + 2.5f;   // KWo - 02.01.2010
         pBot->f_grenade_check_time = gpGlobals->time + RANDOM_FLOAT (2.0f, 5.0f); // KWo - 24.08.2008
          ...
          || ((pEdict->v.weapons & (1 << CS_WEAPON_FLASHBANG)) == 0))
         {
            ...
            pBot->iAimFlags &= ~AIM_GRENADE;          // KWo - 24.02.2006
            pBot->bUsingGrenade = FALSE;              // KWo - 19.03.2007
            if ((pBot->fTimeWeaponSwitch + 1.0f < gpGlobals->time) && (BotGetSafeTask(pBot)->iTask == TASK_THROWFLASHBANG)) // KWo - 22.01.2008
            {
               BotSelectBestWeapon (pBot);
               BotTaskComplete(pBot);
            }
         ...
         else if (pBot->vecGrenade != g_vecZero) // KWo - 22.01.2008 - rewritten again...
         {
            if ((pBot->current_weapon.iId != CS_WEAPON_FLASHBANG) && (pBot->current_weapon.iId != CS_WEAPON_INSWITCH)
                 && (pBot->fTimeWeaponSwitch + 0.5f < gpGlobals->time) /* && !FStrEq(STRING(pEdict->v.viewmodel),"models/v_flashbang.mdl") */)
            {
      ...
      case TASK_THROWSMOKEGRENADE:
      {
         ...
         pBot->f_moved_distance = 15.0f;               // KWo - 25.05.2010
         pBot->fNoCollTime = gpGlobals->time + 2.5f;   // KWo - 02.01.2010
         pBot->f_grenade_check_time = gpGlobals->time + RANDOM_FLOAT (5.0f, 7.0f); // KWo - 24.08.2008
         ...
         if (pBot->vecThrow != g_vecZero) // KWo - 26.07.2007
         {
            pBot->vecGrenade = VecCheckThrow  (pBot, GetGunPosition (pEdict), v_src, 1.0f);
            ...
            || ((pEdict->v.weapons & (1 << CS_WEAPON_SMOKEGRENADE)) == 0))
         {
            ...
            pBot->iAimFlags &= ~AIM_GRENADE;          // KWo - 24.02.2006
            pBot->bUsingGrenade = FALSE;              // KWo - 19.03.2007
            if ((pBot->fTimeWeaponSwitch + 1.0f < gpGlobals->time) && (BotGetSafeTask(pBot)->iTask == TASK_THROWSMOKEGRENADE)) // KWo - 22.01.2008
            {
               BotSelectBestWeapon (pBot);
               BotTaskComplete(pBot);
            }
      ...
      case TASK_SHOOTBREAKABLE:
      {
         ...
         if ((pBot->current_weapon.iId == CS_WEAPON_KNIFE) && ((pEdict->v.origin - v_src).Length() > 40.0f)) // KWo - 20.05.2010
         {
            pBot->bCheckTerrain = TRUE;
            pBot->bMoveToGoal = TRUE;
         }
      ...
      case TASK_PICKUPITEM:
      {
         ...
         if (BotGetSafeTask(pBot)->fTime == 0.0f)
         {
            BotGetSafeTask(pBot)->fTime = gpGlobals->time + ((pBot->iPickupType == PICKUP_HOSTAGE)? 8.0f : 4.0f); // KWo - 08.03.2010
            ...
            if (BotGetSafeTask(pBot)->iTask == TASK_PICKUPITEM) // KWo - 02.09.2006 - against BotRemoveCertainTask
            {
               ...
               if (g_b_DebugEntities)
               {
                  if (pBot->iPickupType == PICKUP_WEAPON)
                  {
                     ...
                     char vmodelname1[64];
                     _snprintf_s (vmodelname1, sizeof (vmodelname1), sizeof (vmodelname1) - 1, "%s", STRING (pBot->pBotPickupItem->v.model));
                     ALERT(at_logged,"[DEBUG] BotExecuteTask - TASK_PICKUPITEM - Bot %s was trying to pickup %s; has %s with %d ammo in clip.\n", pBot->name,
                        vmodelname1, weapon_defs[iId].szClassname, pBot->m_rgAmmoInClip[iId]);
         ...
         switch (pBot->iPickupType)
         {
            case PICKUP_WEAPON:
            {
               ...
               _snprintf_s (vmodelname, sizeof (vmodelname), sizeof (vmodelname) - 1, "%s", STRING (pBot->pBotPickupItem->v.model)); // KWo - 20.12.2006
...
//                        if (pBot->current_weapon.iId != cs_weapon_select[iWeaponNum].iId)
                        if (!WeaponIsPrimaryGun(pBot->current_weapon.iId)) // KWo - 25.12.2006
                        {
                           ...
                           if ((pBot->current_weapon.iId != CS_WEAPON_INSWITCH) && (pBot->fTimeWeaponSwitch + 0.5f < gpGlobals->time))
                           {
...
//                        if (pBot->current_weapon.iId != cs_weapon_select[iWeaponNum].iId)
            ...
            case PICKUP_HOSTAGE:
            {
               pBot->iAimFlags |= AIM_ENTITY;
               v_src = GetGunPosition (pEdict);
               if (distance < ((v_dest.z - pEdict->v.origin.z > 20.0f)? 90.0:70.0f)) // KWo - 08.03.2010
               {
            ...
            case PICKUP_PLANTED_C4:
            {
               ...
               if (pBot->bot_team == TEAM_CS_COUNTER)
               {
                  if ((((float)iEnemyNum <  0.8 * (float)iFriendlyNum))
                     ...
                     || ((distance < 80) && (gpGlobals->time > g_fTimeBombPlanted + g_f_cv_c4timer - 12.0f)))  // KWo - 17.10.2008
                  {
                     iTempWpIndex1 = WaypointFindNearestToMove (pEdict, g_vecBomb);
                     if ((iTempWpIndex1 >= 0) && (iTempWpIndex1 < g_iNumWaypoints) && (iTempWpIndex1 != pBot->curr_wpt_index))
                     {
                     ...
                     else if ((distance < 40) && (iTempWpIndex1 == pBot->curr_wpt_index))
                     {
                        ...
                        bottask_t TempTask = {NULL, NULL, TASK_DEFUSEBOMB, TASKPRI_DEFUSEBOMB, -1, 0.0f, TRUE}; // KWo - 21.02.2007
                        BotPushTask (pBot, &TempTask);
                     }
                     else if ((distance < 80) && (iTempWpIndex1 == pBot->curr_wpt_index))
                     {
                        ...
                        pBot->bMoveToGoal = FALSE;
                        pBot->bCheckTerrain = TRUE;
                        if (GetShootingConeDeviation (pEdict, &v_dest) > 0.9f)
                           pBot->f_move_speed = pEdict->v.maxspeed;
                        pBot->f_sidemove_speed = 0.0f;
                        pEdict->v.button |= IN_DUCK;
                     }
            ...
            case PICKUP_DROPPED_C4: // KWo - 28.08.2008
            {
               if (pBot->bot_team == TEAM_CS_TERRORIST)
               {
                  ...
                  if (!(pBot->iAimFlags & AIM_ENEMY) && (GetShootingConeDeviation (pEdict, &v_dest) > 0.9))
                  {
                     ...
                     {
                        pBot->f_move_speed = 0.5f * pEdict->v.maxspeed;
                     }
                     else
                        pBot->f_move_speed = 0.0f;
                     pBot->f_sidemove_speed = 0.0f;
...
//   bEndTask = false;  // KWo - 05.08.2006
   ...
   bHostDebugPossible = ((pHostEdict) && ((g_vecHostOrigin - pEdict->v.origin).Length() < 30.0f) && !(clients[0].iFlags & CLIENT_ALIVE));
   ...
   else if (pBot->bDead)
   {
      ...
      if ((g_b_cv_chat) && (clients[g_i_botthink_index].fDeathTime + 5.0f < gpGlobals->time)) // KWo - 27.03.2010
      {
         if (g_b_DebugChat)
         ...
         if (!BotRepliesToPlayer (pBot) && (g_fLastChatTime + 1.0f < gpGlobals->time)
            && (pBot->f_lastchattime + 3.0f < gpGlobals->time)) // KWo - 10.03.2013
         {
   ...
   else if ((pBot->iBuyCount > 0) && (pBot->iBuyCount < 8) && pBot->b_can_buy)
   {
      ...
      if (pBot->f_spawn_time + 3.0f < gpGlobals->time)
         pBot->iBuyCount = 0;
   }
   else
      bBotMovement = TRUE;
   ...
   if (pBot->bCheckWeaponSwitch && (pBot->f_spawn_time + 4.0f < gpGlobals->time))
   {
      if (BotHasShield (pBot))
      {
         if (BotHasShieldDrawn (pBot))
      ...
      else
      {
         ...
         else
         {
            if (((pBot->current_weapon.iId == CS_WEAPON_M4A1) || (pBot->current_weapon.iId == CS_WEAPON_USP))
                && (pBot->fTimeSilencerSwitch < gpGlobals->time + 30.0f)
...
*/
   ...
   pBot->f_actual_reaction_time += 0.2f;
...
*/
   ...
   if (!(g_b_cv_jasonmode) && !(pBot->bUsingGrenade)
        && (((pBot->iStates & STATE_HEARINGENEMY) && (BotGetSafeTask(pBot)->iTask != TASK_PLANTBOMB)
          ...
          || ((pBot->f_jumptime + 3.0f < gpGlobals->time) && (pBot->f_jumptime + 4.0f > gpGlobals->time))))
        /* && (!pBot->bIsReloading) */
        && (pBot->fTimeWeaponSwitch + ((pBot->iStates & STATE_SEEINGENEMY) ? 1.0f : 3.0f) < gpGlobals->time)) // KWo - 22.01.2012
            BotSelectBestWeapon (pBot);
...
//      ALERT(at_logged, "[DEBUG] BotThink - Bot %s has task deep = %i, path deep = %i.\n", pBot->name, pBot->i_TaskDeep, pBot->i_PathDeep);
   ...
   pEdict->v.angles.z = pEdict->v.v_angle.z = 0.0f;
   ...
   if (pBot->bWantsToFire && !pBot->bUsingGrenade
      && (pBot->f_shoot_time <= gpGlobals->time) && (pBot->f_spawn_time + 0.5f < gpGlobals->time)) // KWo - 10.07.2008
   {
   ...
   if (g_b_DebugTasks || g_b_DebugCombat)
   {
      if (g_b_DebugCombat)
      {
         ...
         if (iId < 32)
         {
            _snprintf_s (szWeapClassname, sizeof(szWeapClassname), sizeof(szWeapClassname) - 1, "%s", weapon_defs[iId].szClassname);
         }
         else
         {
            _snprintf_s (szWeapClassname, 15, 14, "unknown weapon");
         }
...
//            bPrevCrouch = TRUE;
      ...
      {
         pEdict->v.button |= IN_DUCK;
         pBot->f_ducktime = gpGlobals->time + 1.0f; // KWo - 22.01.2012
      }
      ...
      if ((!(paths[pBot->curr_wpt_index]->flags & W_FL_LADDER)) &&  bPrevLadder
          ...
          && (pBot->f_move_speed > 50.0f) && (pEdict->v.velocity.Length() < 50.0f)) // KWo - 09.04.2016
      {
         pEdict->v.button |= IN_JUMP;
         pBot->f_jumptime = gpGlobals->time + 1.0f;
      }
...
// KWo - check if the direction change is really strong and near WP with low radius
            ...
            && (!(pBot->curr_travel_flags & C_FL_JUMP)))  // KWo - 18.04.2016
      {
         ...
         && (fDistance < paths[pBot->curr_wpt_index]->Radius + 20.0f))
         {
            if ((pBot->pWaypointNodes->NextNode->iIndex >= 0) && (pBot->pWaypointNodes->NextNode->iIndex < g_iNumWaypoints))
            {
               ...
               if (fDot > -0.6f)
                  pBot->f_wpt_tim_str_chg = gpGlobals->time + 1.0f;
            }
         }
      }
            ...
            && (pEdict->v.flags & (FL_ONGROUND | FL_PARTIALGROUND)))  // KWo - 15.04.2016
      {
         ...
         if (pBot->f_move_speed < 150.0f)
            pBot->f_move_speed = 150.0f;
         else if (pBot->f_move_speed > pEdict->v.maxspeed)
            pBot->f_move_speed = pEdict->v.maxspeed;
//         if (bPrevLadder)
//            pBot->f_move_speed = 80.0f;    // KWo - 24.01.2012
      }
      else if ((pBot->cCollisionState == COLLISION_NOTDECIDED) && (pBot->f_move_speed != 0.0f)
...
/*
         else if (pBot->f_move_speed >= 0.0f)
         {
            if ((pBot->curr_wpt_index != BotGetSafeTask(pBot)->iData) && (pBot->f_move_speed < 80.0f))
               pBot->f_move_speed = 80.0f;
            else if (pBot->f_move_speed < 2.0f)
               pBot->f_move_speed = 2.0f;
         }
*/
      }
      ...
      if (pBot->bInWater) // KWo - 17.10.2006 - reverted back
      {
         ...
         if (vecMoveAngles.x > 60.0f)
            pEdict->v.button |= IN_DUCK;
         else if (vecMoveAngles.x < -48.0f) // KWo - 28.04.2010
            pEdict->v.button |= IN_JUMP;
      }
   }
   ...
   if (((pBot->bCheckTerrain) || (pBot->cCollisionState == COLLISION_PROBING) || (pBot->f_probe_time >= gpGlobals->time))
      ...
      && (g_fTimeRoundEnd < gpGlobals->time)) ? (g_f_cv_FreezeTime + 0.0f):(0.2)) < gpGlobals->time)) // KWo - 28.05.2010
   {
      BotCheckTerrain(pBot);
   }

   if (pBot->f_probe_time + 1.0f < gpGlobals->time)
   {
      pBot->bPlayerCollision = FALSE; // KWo - 11.07.2006
   }

   if ((pBot->bHitDoor) && (pBot->f_timeHitDoor + 5.0f < gpGlobals->time)) // KWo - 17.01.2010
   {
      pBot->bHitDoor = FALSE;
   }
...
// moved here writing the new time to check if the bots gets stuck or not
   if (pBot->prev_time <= gpGlobals->time)
   {
      pBot->prev_time = gpGlobals->time + 0.3f; // KWo - 19.03.2010
   }
   ...
   if (pBot->cAvoidGrenade != 0)
   {
      ...
      pEdict->v.button &= ~IN_DUCK;
      pBot->iCampButtons = 0; // KWo - 17.02.2008
      pBot->f_ducktime = 0.0f; // KWo - 17.02.2008

      v_angles = pEdict->v.v_angle;
      MAKE_VECTORS (v_angles);  // KWo - 28.08.2008
      Vector vecForward = (gpGlobals->v_forward * (-pEdict->v.maxspeed)) * 0.2f;
      Vector vecSide = (gpGlobals->v_right * pEdict->v.maxspeed * pBot->cAvoidGrenade) * 0.2f;
...
/*
      if (pHostEdict)
      {
         ...
         g_hudset.y = 0.8f;
//       g_hudset.a1 = 1;
         g_hudset.holdTime = 0.1f;
         g_hudset.channel = 2;
         float f_Angle = sqrt((fabsf(pBot->pEdict->v.punchangle.y) * M_PI / 180.0f) * (fabsf(pBot->pEdict->v.punchangle.y) * M_PI / 180.0f)
            + (fabsf(pBot->pEdict->v.punchangle.x) * M_PI / 180.0f) * (fabsf(pBot->pEdict->v.punchangle.x) * M_PI / 180.0f));  // KWo - 06.04.2010
         float f_Distance = 9999.0f;
         if (!FNullEnt(pBot->pBotEnemy))
...
//      if (pBot->pEdict->v.maxspeed <= 0.0f) // KWo - 14.04.2016
      if (pBot->f_move_speed <= 0.0f) // KWo - 14.04.2016
         fReachWpDelay = 5.0f * fDistance / 240.0f;
      else
//         fReachWpDelay = 5.0f * fDistance / pBot->pEdict->v.maxspeed; // KWo - 14.04.2016
         fReachWpDelay = 5.0f * fDistance / fabs(pBot->f_move_speed); // KWo - 14.04.2016
      if ((paths[pBot->curr_wpt_index]->flags & W_FL_CROUCH) || (paths[pBot->curr_wpt_index]->flags & W_FL_LADDER) || (pBot->pEdict->v.button & IN_DUCK))
         fReachWpDelay *= 3.f;
      if (fReachWpDelay < 3.0f)
         fReachWpDelay = 3.0f;
      if (fReachWpDelay > 10.0f)
         fReachWpDelay = 10.0f;
   }
   if ((pBot->f_jumptime + 10.0f > gpGlobals->time) || (pBot->f_probe_time + 3.0f > gpGlobals->time))
      fReachWpDelay = 10.0f;
...
//         if (g_iFrameCounter == g_i_botthink_index)
      ...
      pBot->pBotPickupItem = NULL;
      pBot->iPickupType = PICKUP_NONE;
      pBot->f_itemcheck_time = gpGlobals->time + 5.0f;
      pBot->pShootBreakable = NULL;
   }
...
*/
   if (pBot->f_jumptime > gpGlobals->time) // KWo - 09.04.2016
   {
      ...
      {
         pEdict->v.button |= IN_DUCK;
//         pBot->f_ducktime = gpGlobals->time + 1.0f; // KWo - cannot be here - otherwise jumping is not working correctly
      }
   }
   if ((pEdict->v.movetype == MOVETYPE_FLY) || (pEdict->v.velocity.z > -50.0f)
      || (pEdict->v.flags & (FL_ONGROUND | FL_PARTIALGROUND))) // KWo - 05.03.2010 (parachute support :) )
      pBot->f_falldowntime = 0.0f;
   else if (pBot->f_falldowntime == 0.0f)
      pBot->f_falldowntime = gpGlobals->time;

   if ((pBot->f_falldowntime + 0.5f < gpGlobals->time)
       && (pBot->f_falldowntime != 0.0f) && g_b_cv_Parachute) // KWo - 07.03.2010 (parachute support :) )
      pEdict->v.button |= IN_USE;
...
/*
   ...
   UTIL_ClampAngle (&pEdict->v.ideal_yaw);
*/
   assert ((pEdict->v.v_angle.x >= -180.0f) && (pEdict->v.v_angle.x < 180.0f)
           && (pEdict->v.v_angle.y >= -180.0f) && (pEdict->v.v_angle.y < 180.0f)
           && (pEdict->v.angles.x >= -180.0f) && (pEdict->v.angles.x < 180.0f)
           && (pEdict->v.angles.y >= -180.0f) && (pEdict->v.angles.y < 180.0f));

// This is PMB method of computing msec value
//   f_msecval = gpGlobals->frametime * 1000.0f; // KWo - 17.03.2007

//   f_msecval = static_cast <byte> ((gpGlobals->time - m_fMsecInterval) * 1000.0f);
// this is Leon Hartwig's method for computing the msec value
   f_msecval = (gpGlobals->time - pBot->fTimePrevThink2) * 1000.0f;
   pBot->fTimePrevThink2 = gpGlobals->time;
...
void BotFreeAllMemory (void)
{
   ...
   {
      g_fWPDisplayTime[i] = 0.0f;
      g_fPathDisplayTime[i] = 0.0f; // KWo - 05.01.2008
   }
